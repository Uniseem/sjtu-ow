...
next: claude
updated: 2026-09-26
blocked_on: 无。社区成立日期、QQ 群链接等你给（后台「全站设置 → 站点信息」填，空着首页不显示这两项）
```

## 现在该谁动手

**Claude**：**前台视觉第二次重做（M9，用户 2026-09-26 定的 Material 3 方向）**。081 完成了设计文档 v3.0 和全站的底子（令牌、组件、磨砂胶囊顶栏、校徽、错误页、样张页），082 首页（M 版）已落地，接着做其余页面的版式（083 起）。计划见「我这边，按顺序」第 22 条起。

**你**：**社区成立日期**和 **QQ 群链接**（没有也不挡：空着时首页不显示这两项，以后在后台「全站设置 → 站点信息」填）；看一眼本地首页 http://localhost:8000 。测试机升级（SSH 密钥在你那边）等 M9 做完再一起升。
...
| M9 前台视觉 v3.0 | 进行中 | 081 设计体系（完成）；082 首页（完成）；083 刊物页；084 赛场与名册；085 个人中心与表单。决定见「你已经拍板的」第 17 条 |
