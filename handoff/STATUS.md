...
```yaml
milestone: M9 前台视觉 v3.0（Material 3），M7 上线准备暂停
round: 081-material3-system
next: claude
updated: 2026-09-26
blocked_on: 无。082 首页要用的社区成立日期、QQ 群链接等你给，没给就先空着（空着时首页不显示这两项）
```

## 现在该谁动手

**Claude**：**前台视觉第二次重做（M9，用户 2026-09-26 定的 Material 3 方向）**。081 完成了设计文档 v3.0 和全站的底子（令牌、组件、磨砂胶囊顶栏、校徽、错误页、样张页），接着做 082 首页（M 版），再做其余页面的版式。计划见「我这边，按顺序」第 22 条起。

**你**：给 082 用的**社区成立日期**和 **QQ 群链接**（没有也不挡：空着时首页不显示这两项，以后在后台「全站设置」填）。测试机升级（SSH 密钥在你那边）等 M9 做完再一起升。

M0–M6、M8 完成；M7 上线准备里代码这边能做的都做了，剩下的要真实服务器或你来定（见「服务器上」）。
...
**CI 是绿的**（045 起每次推送都通过）。074–079 原在分支 `claude/nifty-planck-i1qe4u` 上，**080 已快进合并到 `main`，CI 通过，分支已删**。

### 合并分支、升级测试机（078 写的步骤，079 核对过，要你来做）

**1. ~~合并到 `main`~~** —— **080 已完成**。当时的命令留作记录：

```bash
git fetch origin
git checkout main && git merge --ff-only origin/claude/nifty-planck-i1qe4u
git push origin main        # 推完看一眼 GitHub Actions，红了先修
```

**2. 先弄清 SSH 主机密钥为什么变了**（2026-09-25 发现）。如果服务器重装过，`/srv/sjtu-ow` 和 `.env` 可能都没了：按 README「生产 / 测试环境启动」从头部署，三把密钥重新生成（旧的加密数据就解不开了，测试机上没有真实数据，问题不大）。没重装过的话，确认原因再连。

**3. 从 066 升级到 079**（测试机停在 066；按设计 16.8 的升级流程；079 没有迁移）：

```bash
cd /srv/sjtu-ow
C="docker compose -p sjtu-ow-test -f deploy/docker-compose.yml --env-file .env"
$C exec web python manage.py backup                  # 升级前手动备份一次
git pull --ff-only
$C build
$C stop web worker                                   # 这段时间公开页面照常，其他请求是维护页
$C run --rm --no-deps web python manage.py migrate --plan   # 先看一眼要跑哪些（066 之后有 12 轮）
$C run --rm --no-deps web python manage.py migrate --noinput
$C run --rm --no-deps web python manage.py remove_stale_contenttypes --include-stale-apps --noinput
$C run --rm --no-deps web python manage.py init_site  # 072 起内容编辑要有评论权限
$C up -d
$C exec web python manage.py prerender               # 074–077 换了全部模板，必须全量重新生成
```

- 迁移一共 11 个：067 删开放 API 的三张表、清排队的 Webhook 任务、赛事去掉上游字段（`integrations/0003`、`tournaments/0005`）；068 内战报名的游戏 ID 外键改成置空（`scrims/0002`）；069、070 个人报名和临时队伍（`tournaments/0006`、`0007`）；072、073 评论（`comments/0001`、`0002`，以及 `accounts`、`content`、`moderation` 各一个）；078 入队申请「坦克」的字段说明（`teams/0002`，不动表）
- **定时任务**：`/etc/cron.d/sjtu-ow-test` 里删掉每 10 分钟扫 Webhook 的那一行（`from integrations.tasks import deliver_due_webhooks`；067 起这个模块已删，留着会每 10 分钟报一次 ImportError），其余和 `deploy/crontab.example` 对一遍

**4. 升级后检查**：

- `curl -I http://sjtu.ow-shanghaiuniversity.com/` 是 308 跳到 `https://`；`/healthz` 四项 ok；`/robots.txt` 只有 `Disallow: /`
- **worker 真的能生成**：后台改一篇文章，一分钟后刷新文章页和首页看变了没有；后台「半静态渲染」页没有失败记录（064 的坑：只在 `web` 里跑通不算）
- 缺页兜底（068 补的 `PrerenderMissMiddleware`）：未登录打开一个还没有静态文件的公开页（比如刚发布、还没生成完的文章），页面照常显示，随后后台「半静态渲染」里能看到它被补生成
- **内战的按时刷新**（076）：建一个 10 分钟后截止报名的内战，截止后看 `/scrims/`、首页和详情页是否变成「报名截止」
- **看新设计**：超级管理员登录后打开 `/_styleguide/`（整套组件），再在手机上走一遍首页、赛事、内战、个人中心。觉得哪里不对直接说，改组件是全站一起变
...
23. **082 首页（M 版）**：全屏首屏（色彩渐变底、校徽水印、关键数字条、近期安排卡、快捷入口磁贴）、资讯卡片、通知公告、活动统计、战队；`SiteSettings.founded_on`、`qq_group_url`（迁移）；删除焦点图（`HomePageCarouselItem` 迁移删表，`carousel.js` 删）；13.13.4 新补的三条首页刷新事件逐条核对代码；README 首页维护一段
24. **083 刊物页**：资讯列表、文章页、搜索、普通页面按 v3.0 的版式（筛选标签、圆角封面、正文和评论区）
25. **084 赛场与名册**：赛事、内战、战队的列表和详情（色调区块页头、票根卡片）、成员展示（去掉编号名册里 v2.0 的写法）
26. **085 个人中心与表单**：`/me/` 各页、登录注册（去掉「注册以后可以」上的粗线）、报名和战队表单；收尾：REVIEW-GUIDE、测试机升级步骤
...
| M9 前台视觉 v3.0 | 进行中 | 081 设计体系（完成）；082 首页；083 刊物页；084 赛场与名册；085 个人中心与表单。决定见「你已经拍板的」第 17 条 |
| M8 独立版功能 | **已完成** | 067 去掉上游、报名自动通过；068 顺带修复；069 个人报名；070 编队与临时队伍；071 站内搜索；072 文章评论基础；073 评论增强；**前台重做** 074 设计体系、075–077 各页面；078 上线前收尾；079 文字对比度。决定见「你已经拍板的」第 15、16 条。Claude 实现 + 自查，待独立复核（REVIEW-GUIDE 有 074–079 一节） |
