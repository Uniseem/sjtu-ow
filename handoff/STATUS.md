...
008–039 可以交给 Grok 做独立复核。**先读 `handoff/REVIEW-GUIDE.md`**——30 轮逐个读成本太高，那份指南按「哪里最可能还有问题」排了优先级，并列出了我自己判断错过的实例和拍板的主观取舍。结论追加到对应轮次的 `review.md`。
...
039 轮的变异测试找到 **3 个真实的测试空白**，都没修（收尾时只做记录）。按严重程度：

1. **预渲染的 Set-Cookie 闸没有测试**（010）。`core/prerender.py` 有三道安全闸，这道拆掉没有任何测试会红。它防的是「会给访客设 cookie 的个性化页面被静态化，然后所有人看到同一份」——设计 15.2 明写预渲染页面不能含个人信息。023 轮那条测试验的是「生成的 HTML 里没有敏感内容」，不是「该拒绝的响应被拒绝了」。
2. **签名比对没有断言用 `compare_digest`**（017）。改成 `==` 所有测试照过。时序攻击本身难测，但可以断言代码用的是哪个原语。
3. **解散战队的 `is_disbanded` 检查没有测试**（012）。`teams/services.py` 里有 4 处，改掉第一处（申请入队）没人发现。

细节和修法在 `rounds/039-mutation-sweep/report.md`。
