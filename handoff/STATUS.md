...
11. **068 顺带修复**（各一条测试）：内容编辑改不了文章分类；`restore.py` 加密列漏了 R2 密钥；`/me/registrations/` 的内战占位文案和 `ME_NAV` 入口；`prod.py` 的中间件列表漏了 `PrerenderMissMiddleware`（生产环境预渲染缺页兜底一直是关的）；内战把段位 0 当没填；已结束内战的游戏 ID 删除会 500；`mutate_guards.py` 的应用列表
...
| M8 独立版功能 | 进行中 | 067 去掉上游、报名自动通过（完成）；068 顺带修复；069–070 个人报名与编队；071 站内搜索；072–073 文章评论；074 上线前收尾。决定见「你已经拍板的」第 15 条 |
