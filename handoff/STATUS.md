...
```yaml
milestone: M10 前台视觉 v4.0（从零重做），M7 上线准备暂停
round: 086-v4-system-home
next: claude
updated: 2026-09-26
blocked_on: 无。测试机升级要你的 SSH 密钥（先弄清主机密钥为什么变了），等 M10 做完再一起升
```

## 现在该谁动手

**Claude**：**M10 前台视觉 v4.0**（用户 2026-09-26 晚上决定：之前所有前台设计作废，从零重做，由 Claude 直接定一套）。086 完成设计文档 v4.0、深浅两套颜色、页头页脚和首页；接着 087 资讯 / 文章 / 搜索，088 赛事 / 内战 / 战队 / 成员，089 个人中心 / 表单 / 评论 / 错误页。计划见「我这边，按顺序」第 27 条起。

**你**：M10 做完后在本地 http://localhost:8000 看整站（浏览器或手机切到深色模式看另一套）。后台「全站设置 → 站点信息」可以上传**首屏图片**；暴雪官方图片只在后台上传，不进仓库。测试机升级（SSH 密钥在你那边）等 M10 做完。
...
### 合并分支、升级测试机（078 写的步骤，079 核对过，085 更新到 085，要你来做）
...
**3. 从 066 升级到 085**（测试机停在 066；按设计 16.8 的升级流程；079–081、083–085 没有迁移）：
...
$C run --rm --no-deps web python manage.py migrate --plan   # 先看一眼要跑哪些（066 之后有 19 轮）
...
$C up -d
$C exec web python manage.py prerender               # 074–085 换了全部模板，必须全量重新生成
```

- 迁移一共 13 个：067 删开放 API 的三张表、清排队的 Webhook 任务、赛事去掉上游字段（`integrations/0003`、`tournaments/0005`）；068 内战报名的游戏 ID 外键改成置空（`scrims/0002`）；069、070 个人报名和临时队伍（`tournaments/0006`、`0007`）；072、073 评论（`comments/0001`、`0002`，以及 `accounts`、`content`、`moderation` 各一个）；078 入队申请「坦克」的字段说明（`teams/0002`，不动表）；082 删焦点图表（`content/0005`）、站点设置加成立日期和 QQ 群链接（`core/0012`）
...
28. **087 刊物页**：资讯列表（图片卡网格、筛选）、文章页（正文栏居中、头图、同栏目最新放正文后面）、普通页面、搜索、投稿入口
29. **088 赛场与名册**：赛事、内战、战队的列表和详情（大图横幅、图片卡、表格）、成员展示；去掉 `.on-tonal` 和 v3 的票根、图块样式
30. **089 个人中心、表单、评论、错误页**：`/me/`、登录注册、报名和战队表单、评论区、错误页（深色）；删掉 v3 令牌别名和剩下的旧样式；收尾 REVIEW-GUIDE、测试机升级步骤
...
| M10 前台视觉 v4.0 | 进行中 | 086 设计体系与首页（完成）；087 刊物页；088 赛场与名册；089 个人中心、表单、评论、错误页 |
