...
**Claude**：用户外出期间由 Claude 连做（实现 + 自查）。M2、M3、M4 都已完成。下一个里程碑是 **M5 开放 API 与 Webhook**（设计第 11 章、12.11–12.13），建议拆成 3 轮：API 客户端与签名认证、读接口与写接口、Webhook 投递与接口文档。

用户回来后，008–016 可以交给 Grok 做独立复核：读各自的 `report.md` 和 `008-m2-fonts/request.md` 末尾的复核清单，自己跑命令验证，把结论追加到对应的 `review.md`。
...
| M5 开放 API 与 Webhook | 下一个 | 设计第 11 章、12.11–12.13 |
| M6 内战 | 未开始 | |
| M7 上线准备 | 未开始 | |
