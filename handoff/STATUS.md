...
```yaml
milestone: M7 上线准备（M8 独立版功能 079 完成）
round: 079-contrast
next: user
updated: 2026-09-26
blocked_on: 分支合并到 main、测试机升级（SSH 密钥在你那边），见「下次开工的第一件事」
```

## 现在该谁动手

**你**：**M8 全部完成**（067–073 功能、074–077 前台重做、078 收尾、079 对比度）。下一步要你来：把分支 `claude/nifty-planck-i1qe4u` 合并到 `main`（CI 才会跑）、升级测试机、在测试站上看新设计。步骤见下面「下次开工的第一件事」。看完有要改的直接说，Claude 接着按轮次做。

**Claude**：M0–M6、M8 完成；M7 上线准备里代码这边能做的都做了，剩下的要真实服务器或你来定（见「服务器上」）。
...
**`main` 上的 CI 到 073 都是绿的**（045 起每次推送都通过）。**074–079 在分支 `claude/nifty-planck-i1qe4u` 上**（这个会话只能推这个分支），CI 只在 `main` 上跑，所以这六轮的 CI 还没跑过；每轮都在本地跑了和 CI 一样的整组检查。

### 合并分支、升级测试机（078 写的步骤，079 核对过，要你来做）

**1. 合并到 `main`**：分支是在 073 上直接接的六个提交（074–079），可以快进合并：
...
| M7 上线准备 | 进行中 | 022–031 运维命令、第 15 章与附录 A/B/C 核查、地址与错误码核查、异地加密备份已完成；033–037 状态表、规模、并发、升级演练已完成；039–042 测试有效性（变异测试）完成。2026-09-25 起暂停等 M8，**M8 已在 079 做完，恢复**；合并分支、测试机升级（078 写好了步骤）、国内网络测试、协议正文、试运行等你 |
