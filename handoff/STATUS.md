...
16. **前台视觉整体重做（2026-09-26）** → 主色仍是交大红，其余布局和设计全部推倒重来；参考其他游戏、社区、组织的官网，形成一套完整的体系。又强调「放弃掉当前的设计，不要被影响，当前的设计完全不行」：065 照交大官网的风格作废，不沿用任何元素。→ **074 完成设计体系**（设计 v2.0 的 13.2 节、样式表重写、外框、组件、样张页 `/_styleguide/`、错误页）；075–077 改各页面，**077 做完前页面内容区是半成品，不要部署**
...
16. **075 首页与内容页**：首页五个区块（头条焦点图 + 近期、资讯、赛事与内战日程带、战队、参与）、资讯列表、文章页（封面、同栏目最新）、普通页面、评论区、搜索结果；焦点图脚本重写
17. **076 赛场和名册**：赛事列表与详情、个人报名、报名详情、内战列表与详情、战队列表与主页、成员展示
18. **077 个人中心与表单**：`/me/` 各页、登录注册（allauth）、报名、创建战队、申请入队；去掉 daisyUI 插件，加「前台模板里不再有 daisyUI 类名」的测试
19. **078 上线前收尾**（原 074）：更新上线清单、REVIEW-GUIDE，测试机重新部署（先解决 SSH 主机密钥变化；迁移后跑 `remove_stale_contenttypes`，删 cron 里的 webhook 行）；保留地址片段补上 `search`；设计 13.13.3 账号区域那一行的旧说法。**本会话没有测试机的 SSH 密钥**，部署要在你那边做或给权限
...
| M8 独立版功能 | 进行中 | 067 去掉上游、报名自动通过（完成）；068 顺带修复（完成）；069 个人报名（完成）；070 编队与临时队伍（完成）；071 站内搜索（完成）；072 文章评论基础（完成）；073 评论增强（完成）；**前台重做** 074 设计体系（完成）、075–077 各页面；078 上线前收尾。决定见「你已经拍板的」第 15、16 条 |
