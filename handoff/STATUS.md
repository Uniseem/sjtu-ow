...
4. ~~**跑完 042 剩下的 178 个守卫**~~ —— **059 已完成**：218 个守卫全部有结论。补了 69 条测试（大约一半是权限类），修了一个真 bug（重复入队申请的取消被事务回滚），停用账号的报名提示改回设计
   ...
   - **062 要做**：赛事、战队、内战详情页的分享信息（设计 13.14 的表）没做，`og:title` 等都是全站默认——`content/seo.py` 里写着「Extension point for later milestones」。社团链接主要在 QQ 群传播，预览不好看会影响点击
5. ~~你拍板之后：账号注销和导出、两步验证、起草协议~~ —— 注销和导出 **058 已完成**；两步验证上线先不做；协议 **060 已起草**，要社团填【】
6. **运维上新发现的两件**（060 起草隐私政策时核对出来的）：异地备份上传到 R2 后**没有清理**，会一直留着，隐私政策要写保存期限，得先定；Docker 日志没设大小上限，时间长了会一直涨
...
- M2–M4 留下的「以后再接」的占位，已经发现四处没接（055 内战权限、056 首页和参赛记录、059 解散限制）。`blocked_query()` 已删（061）；`content/seo.py` 的占位是真没做（→ 062）
