...
**Claude**：用户外出期间由 Claude 连做（实现 + 自查）。M2–M4 已完成；M5 的 017（客户端与签名认证）已完成。下一轮是 **018 业务接口**（设计 11.4–11.7：赛事、报名、名单、统计、审核、CSV 导出，含展开项、字段筛选、游标分页和 `updated_since`）。

用户回来后，008–017 可以交给 Grok 做独立复核：读各自的 `report.md` 和 `008-m2-fonts/request.md` 末尾的复核清单，自己跑命令验证，把结论追加到对应的 `review.md`。
...
| M5 开放 API 与 Webhook | 进行中 | 017 客户端与签名认证已完成；018 业务接口、019 Webhook 与文档未开始 |
| M6 内战 | 未开始 | |
| M7 上线准备 | 未开始 | |
