...
```yaml
milestone: M7 上线准备
round: 063-log-and-backup-retention
next: claude
updated: 2026-09-18
blocked_on: 无
```
...
7. **064：worker 容器没挂静态文件卷，事件触发的页面重新生成在服务器上全部失败**（063 部署时发现）。发布文章、改赛事、报名之后静态页一直是旧的；任务记为成功、`/healthz` 看不出来。修好后要在测试机上**真的触发一次**、看静态页变了，并考虑让这类失败能被看见
