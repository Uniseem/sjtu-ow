...
4. ~~**跑完 042 剩下的 178 个守卫**~~ —— **059 已完成**：218 个守卫全部有结论。补了 69 条测试（大约一半是权限类），修了一个真 bug（重复入队申请的取消被事务回滚），停用账号的报名提示改回设计
   - **061 要做**：`disband_blockers()` 永远返回空——设计 7.5 要求有有效报名的战队不能解散，**现在能解散**。M4 留的占位，没人回来接
...
- M2–M4 留下的「以后再接」的占位，已经发现四处没接（055 内战权限、056 首页和参赛记录、059 解散限制）。`blocked_query()`、`content/seo.py` 的「Extension point for later milestones」还要看一眼
