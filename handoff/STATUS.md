...
round: 080-windows-test-path
next: user
updated: 2026-09-26
blocked_on: 测试机升级（SSH 密钥在你那边），见「下次开工的第一件事」
```

## 现在该谁动手

**你**：**M8 全部完成**（067–073 功能、074–077 前台重做、078 收尾、079 对比度）。**080 已合并到 `main`，CI 绿，分支已删；本地开发服务器上可以先看新设计。** 下一步要你来：升级测试机、在测试站上看新设计。步骤见下面「下次开工的第一件事」。看完有要改的直接说，Claude 接着按轮次做。
