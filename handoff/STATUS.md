...
**你的目标是尽快上线**（2026-09-18）。下面这些要你决定，括号里是我的建议：

1. **测试机上的部署算测试环境还是正式站**（先当测试环境：设计 16.10 要求显示「测试环境」横幅、禁止搜索引擎抓取，这两样要先做）
2. **两步验证**（超级管理员强制，其他后台角色建议开。可能要加 allauth 的附加依赖，加之前查许可证）
3. **账号注销和导出**（按设计建议：注销后匿名化，导出 JSON。设计 15.3 要求有注销渠道，隐私政策也要写，不做不好上线）
4. **「高风险内容暂缓公开」开关**（先不做，默认本来就是关的）
5. **视觉风格**（先用现在的主题上线，只换 logo 和主色）
6. **错误追踪**（先确认出错时能不能给管理员发邮件，够用就不接 Sentry，接 Sentry 要加依赖）
7. **用户协议和隐私政策**（我按网站实际收集的数据起草一版，你们改）
8. **测试机的 SSH 密码登录**（关掉，只留密钥。我没动：这是改系统安全配置，改错会把自己锁在外面）
...
**上线日期主要取决于你那边的外部资源什么时候到位，加上试运行几天。** 我这边的活不用等，两边同时开工。
...
| 用户协议、隐私政策正文 | 未开始。见「还没定的」第 7 条 |
| R2 存储桶和 `BACKUP_ENCRYPTION_KEY`、DeepSeek API Key | 未开始 |

### 我这边，按顺序

1. **补安全相关的测试空白**：042 找到的 14 处，加密那 3 处最先（清单在 `rounds/042-guard-sweep/report.md`），补完逐个确认测试会失败
2. **测试环境的横幅和禁止抓取**（设计 16.10）、**微信内置浏览器提示**（设计 16.9，043 之后才发现没实现）
...
6. **小范围试运行**：10 到 20 个社团成员用几天，把注册、战队、组队大厅、内战都走一遍
7. QQ 群正式公告

### 上线前不做

上游对接（他们的接口还没写）、「暂缓公开」开关、完整视觉定稿、Litestream、访问统计、全套独立复核（可以同时让 Grok 做，但不等它）。
...
| 16.9：在微信内置浏览器里打开时，页面顶部提示「点右上角菜单，在浏览器中打开」（**默认**） | **没实现**，代码里搜不到 `MicroMessenger`（050 前规划时发现） | 不用 |
