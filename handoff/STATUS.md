...
008–041 可以交给 Grok 做独立复核。**先读 `handoff/REVIEW-GUIDE.md`**——30 轮逐个读成本太高，那份指南按「哪里最可能还有问题」排了优先级，并列出了我自己判断错过的实例和拍板的主观取舍。结论追加到对应轮次的 `review.md`。
...
1. ~~预渲染的 Set-Cookie 闸没有测试~~ —— **040 已补**
2. ~~签名比对没有断言用 `compare_digest`~~ —— **041 已补**
3. ~~解散战队的检查没有测试~~ —— **041 已补**。039 说的是「一处」，041 逐个变异发现**四处全都没测试**，四处都补了

039 只试了 14 个变异点，008–017 还有很多没试过。`REVIEW-GUIDE.md` 里的建议仍然成立。
