...
**043 核对设计时翻出 4 条要你拍板的**：两步验证强制与否、账号注销与导出怎么做、「暂缓公开」开关要不要做、`main` 分支保护什么时候开。详见下面「设计里还没实现的」。
...
| 17.5：`main` 分支受保护、只能通过合并请求合入、至少 1 人审阅 | 实际做法是每轮直接提交到 `main`。仓库在 GitHub 上了，可以开分支保护 | **要**：现在就开，还是有第二个开发者再开 |
