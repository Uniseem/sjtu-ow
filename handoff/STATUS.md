...
0. **CI 还是红的**：GitHub 上第一次跑 CI 时 `/healthz` 两条测试返回 503（本地是 200），原因还没确认。044 让测试失败时打印完整响应，看 044 那次 CI 的输出定位原因再修（见 `rounds/044-ci-and-main-policy/report.md`）
