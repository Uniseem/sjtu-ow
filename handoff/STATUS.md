...
9. **看一眼成员展示**（066）：<https://sjtu.ow-shanghaiuniversity.com/members/>。「加入的用户」我定的是「账号没停用、验证过邮箱」；成员页对所有人公开。有不同意见告诉我
10. 我这边的下一件：内容编辑其实改不了文章分类（设计说能，`init_site` 从没给过权限，066 发现）；然后手动补验「`messages.error` + `redirect`」式的守卫
