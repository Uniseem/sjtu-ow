...
**Claude**：用户外出期间由 Claude 连做（实现 + 自查）。M2、M3 已完成；M4 拆成三轮，014 赛事本身已完成。下一轮是 **015 报名事务与状态机**（设计 8.3–8.6、12.8.2–12.8.4），开头先把赛事的详细说明换成富文本（014 复核的 A1）。

用户回来后，008–014 可以交给 Grok 做独立复核：读各自的 `report.md` 和 `008-m2-fonts/request.md` 末尾的复核清单，自己跑命令验证，把结论追加到对应的 `review.md`。
...
| M4 赛事与报名 | 进行中 | 014 赛事已完成；015 报名、016 后台审核与邮件未开始 |
