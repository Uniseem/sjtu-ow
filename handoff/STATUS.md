...
next: user
updated: 2026-09-26
blocked_on: 你看一遍 v4.0 全站（浅色、深色、手机）；测试机升级要你的 SSH 密钥（先弄清主机密钥为什么变了）
```

## 现在该谁动手

**Claude**：**M10 前台视觉 v4.0 做完了**（086–089，用户 2026-09-26 晚上决定从零重做、由 Claude 直接定一套）：设计体系和首页、资讯 / 文章 / 搜索、赛事 / 内战 / 战队 / 成员、个人中心 / 表单 / 评论 / 错误页。等你看过再改。

**你**：① 本地 http://localhost:8000 把全站走一遍，浏览器或手机切到深色模式再看一遍；哪一页哪一块不对，直接说。② 后台「全站设置 → 站点信息」上传**首屏图片**、填社区成立日期和 QQ 群链接；赛事封面、文章封面、队标也在后台传（暴雪的图只在后台传，不进仓库）。③ 升级测试机：步骤见下面「合并分支、升级测试机」（SSH 密钥在你那边，先弄清主机密钥为什么变了）。
...
### 合并分支、升级测试机（078 写的步骤，079 核对过，089 更新到 089，要你来做）
...
**3. 从 066 升级到 089**（测试机停在 066；按设计 16.8 的升级流程；只有 082 和 086 有新迁移）：
...
$C run --rm --no-deps web python manage.py migrate --plan   # 先看一眼要跑哪些（066 之后有 23 轮）
...
$C up -d
$C exec web python manage.py prerender               # 086–089 换了全部模板，必须全量重新生成
```

- 迁移一共 14 个：067 删开放 API 的三张表、清排队的 Webhook 任务、赛事去掉上游字段（`integrations/0003`、`tournaments/0005`）；068 内战报名的游戏 ID 外键改成置空（`scrims/0002`）；069、070 个人报名和临时队伍（`tournaments/0006`、`0007`）；072、073 评论（`comments/0001`、`0002`，以及 `accounts`、`content`、`moderation` 各一个）；078 入队申请「坦克」的字段说明（`teams/0002`，不动表）；082 删焦点图表（`content/0005`）、站点设置加成立日期和 QQ 群链接（`core/0012`）；086 站点设置加首屏图片（`core/0013`）
...
| M10 前台视觉 v4.0 | 完成，等用户看 | 086 设计体系与首页；087 刊物页；088 赛场与名册；089 个人中心、表单、评论、错误页。决定见「你已经拍板的」第 18 条 |
