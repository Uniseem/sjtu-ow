...
```yaml
milestone: M7 上线准备
round: 058-account-deletion-and-export
next: claude
updated: 2026-09-18
blocked_on: 无
```
...
4. **跑完 042 剩下的 178 个守卫**：脚本加了续跑参数，**059 正在跑**。已经看到的幸存者里有好几处权限检查没有测试（AI 审核后台、内战分队页、发测试邮件、只有队长能改战队资料、只有队长能拒绝入队申请），防 SSRF 的地址检查和预渲染路径的 `..` 检查也没测到
5. ~~你拍板之后：账号注销和导出、两步验证、起草协议~~ —— 注销和导出 **058 已完成**；两步验证上线先不做；协议 **060 起草中**
