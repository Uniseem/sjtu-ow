...
```yaml
milestone: M7 上线准备
round: 056-home-and-team-records
next: claude
updated: 2026-09-18
blocked_on: 无
```
...
3. ~~**部署到测试机**~~ —— **053 已部署，054 修好了部署时发现的问题**：重启策略（杀进程实测会自动重启）、健康检查（healthy）、定时任务（测试机装了 cron，按时执行已验证）、全量预渲染已跑
   ...
   - ~~搜一遍「以后再做」式的占位~~ —— **056 搜了**：首页的赛事、内战两块和战队的参赛记录**一直没做**，已补上；设计 13.13.4 的重新生成事件表 14 行里 8 行没做全，056 补了和首页、参赛记录相关的，**还剩 057**：
     - 内战报名、取消报名 → 活动详情（报名统计）
     - 用户改昵称 → 所在战队、报名的内战、署名的文章
     - 文章变化 → 关联赛事的详情页
     - 游戏模式修改 → 组队大厅外壳
     - 后台全站设置里还标着「后续里程碑使用」的参数（这些早就在用了）
