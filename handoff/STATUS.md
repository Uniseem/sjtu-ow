...
1. **两步验证**（超级管理员强制，其他后台角色建议开。可能要加 allauth 的附加依赖，加之前查许可证）
2. **账号注销和导出**（按设计建议：注销后匿名化，导出 JSON。设计 15.3 要求有注销渠道，隐私政策也要写，不做不好上线）
3. **「高风险内容暂缓公开」开关**（先不做，默认本来就是关的）
4. **视觉风格**（先用现在的主题上线，只换 logo 和主色）
5. **错误追踪**（先确认出错时能不能给管理员发邮件，够用就不接 Sentry，接 Sentry 要加依赖）
6. **用户协议和隐私政策**（我按网站实际收集的数据起草一版，你们改）
