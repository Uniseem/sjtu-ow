...
round: 085-account-forms
next: user
updated: 2026-09-26
blocked_on: 你看一遍 v3.0 全站；测试机升级要你的 SSH 密钥（先弄清主机密钥为什么变了）。社区成立日期、QQ 群链接在后台「全站设置 → 站点信息」填，空着首页不显示这两项
```

## 现在该谁动手

**Claude**：**M9 前台视觉 v3.0 做完了**（081–085，用户 2026-09-26 定的 Material 3 方向）：设计体系、首页（M 版）、资讯 / 文章 / 搜索、赛场与名册、个人中心与表单。等你看过再改；你觉得哪里不对，直接说哪一页哪一块。

**你**：① 本地 http://localhost:8000 把全站走一遍（首页、资讯、赛事、内战、战队、成员、登录注册、个人中心）；② **社区成立日期**和 **QQ 群链接**（空着时首页不显示这两项，后台「全站设置 → 站点信息」填）；③ 首页近期安排里赛事写「已通过 N 队」，要不要改成含待审核的「已报名 N 队」；④ 升级测试机：步骤见下面「合并分支、升级测试机」（SSH 密钥在你那边，先弄清主机密钥为什么变了）。
...
| M9 前台视觉 v3.0 | 完成，等用户看 | 081 设计体系；082 首页；083 刊物页；084 赛场与名册；085 个人中心与表单。决定见「你已经拍板的」第 17 条 |
