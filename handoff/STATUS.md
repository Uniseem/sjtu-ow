...
用户回来后，008–023 可以交给 Grok 做独立复核：读各自的 `report.md` 和 `008-m2-fonts/request.md` 末尾的复核清单，自己跑命令验证，把结论追加到对应的 `review.md`。
...
5. **AI 审核记录的「已处理」以哪个字段为准？**（023）设计 15.5 要求「已处理的保留 180 天，未处理的一直保留」。后半截已经成立（清理任务不碰审核记录），前半截没实现——需要先确认判定字段，确认了就能接进 `cleanup_old_data`。
6. **内战手动调整要不要做成拖拽？**（021）设计 9.5 的原话是「拖拽玩家可以换队」，我做成了下拉选择：功能等价、键盘可用、不依赖库，但确实不是设计写的样子。这是我主动的取舍，不是做不到（`Sortable.min.js` 已经在项目里）。要拖拽的话补一轮。

另外 019 留了一个自己认的缺口：Webhook 24 小时那一档重试**没有真实验证过**（`deliver_due_webhooks` 兜底任务 022 已经接到 crontab 上了，每 10 分钟扫一次）。详见 `rounds/019-m5-webhooks/review.md`。
...
| M7 上线准备 | 进行中 | 022 运维命令、023 第 15 章逐条核查已完成；**备份加密与异地同步、部署、试运行等待用户** |
