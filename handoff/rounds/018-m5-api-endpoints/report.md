...
1. **`review_not_allowed` 和 `invalid_state_transition` 两个错误码**按设计 11.6.7 实现。

   > **030 轮更正**：我当时写「11.4 的错误码总表里没列」，这是错的。错误码总表在 **11.10** 不是 11.4，两个码本来就在表里。是我去错了章节就下了结论。030 轮真去比对了 11.10 的全表，发现了 5 处别的不符（4 个码没实现、`validation_error` 状态码错），都修了。
...
2. ~~两个错误码是否补进设计 11.4 的总表~~ —— **030 轮更正：表在 11.10，两个码本来就在，这条问题不成立**。
