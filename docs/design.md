...
| 栏目头 | `c-pagehead` | 一级标题、导语、元信息、操作。不写英文眉标。`on-tonal` 里用于赛场页 |
...
| 资料卡 | `c-facts--card` | 详情页侧栏的资料表放在描边大圆角卡里 |
| 位置统计 | `c-rolestats` | 内战详情：坦克、输出、支援三张 `surface-low` 小卡 |
| 名册卡 | `c-roster` | 成员页「全部成员」：每人一张小卡，首字头像、昵称、所在分组、右上角编号 |
| 侧栏菜单 | `c-sidenav` | 个人中心左栏（13.5）：每项一个图标加文字，当前项 `primary-container` 底的胶囊；手机上换成 `c-tabs` |
| 账号卡片 | `c-auth` | 登录、注册和其他账号页：最宽 28rem 的描边大圆角（`radius-xl`）卡片，里面是标题、说明和表单 |
| 注册以后可以 | `c-why` | 登录注册卡片右边（只在桌面显示）：`surface-low` 底的大圆角卡，三项各带一个色调图标片，不写编号、不画粗线 |
...
- 左侧（手机端为顶部横向滚动的标签）菜单，每项一个图标（v3.0 起代替 `01`–`07` 编号），当前项用 `primary-container` 底色标出（手机上是 `c-tabs` 的当前项，13.2.7）：基本资料、游戏 ID 与段位、联系方式、我的战队、我的报名、我的内战、账号安全
