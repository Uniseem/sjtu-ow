...
| 功能权限 | 控制普通用户能否使用发车、创建战队、报名等功能的开关（4.3 节） |
...
| `core` | 全站设置（SMTP 等）、游戏模式、字体库与排版设置、页面预渲染、异步邮件后端、公共组件（加密字段、位置 mixin、健康检查） |
...
│   │   └── prod.py
│   └── urls.py
├── accounts/  core/  content/  teams/  lfg/  tournaments/  scrims/  integrations/
│   ├── models.py
│   ├── services.py            业务逻辑
│   ├── views.py               前台视图
...
- **删除游戏 ID 的限制**：如果这个 ID 正在某个未结束内战的报名中使用，不能删除，提示先取消内战报名。使用它的车帖会一起删除；已提交的赛事名单快照不受影响
...
- 除投稿者以外的后台角色，**建议**开启两步验证（allauth 自带 TOTP 功能），是否强制见第 19 章
...
- **单个用户规则**：单独允许或单独禁止某个用户使用某个功能，比如禁止某个违规用户发车
...
| 操作 | 谁可以 | 额外条件 |
|---|---|---|
| 浏览首页、文章、赛事、战队、内战活动 | 所有人（包括访客） | |
| 浏览组队大厅 | 已登录用户 | 车帖会显示游戏 ID，所以要求登录（**默认**） |
| 发车 | 已登录用户 | `lfg_post`；至少有 1 个游戏 ID；未过期车帖不超过上限 |
| 编辑、关闭自己的车帖 | 车主 | |
...
| 区块 | 内容来源 |
|---|---|
| 最新公告 | 自动取最新发布的 3 篇文章，可以在后台指定「置顶文章」覆盖（**默认**） |
| 正在报名的赛事 | 自动取当前处于报名期内的赛事 |
| 近期内战 | 自动取未来 7 天内的内战活动（**默认**） |
| 组队大厅入口 | 已登录用户看到当前招人中的车帖数量（动态加载，13.13.3 节）；未登录访客看到「登录后查看」 |
...
| 图片 | 图片 + 图注 |
| 引用 | 引用文字 + 出处 |
| 视频 | 视频链接嵌入。需要确认 Wagtail 自带的嵌入提供方是否支持 B 站，不支持就自定义嵌入规则 |
...
- **默认服务商 DeepSeek，默认模型 `deepseek-v4.1-flash`**，在后台可以换模型。用 OpenAI 兼容的接口调用（DeepSeek 同时提供 Anthropic 兼容接口）
  ...
  - **关闭思考模式**：审核是分类任务，不需要长时间推理，关掉更快也更便宜；单次输出限制在几百 token
...
- **异步执行**：内容保存后由 worker 调用（事务提交后排队），不拖慢用户提交；接口超时或报错时重试 3 次，仍失败则记为「无法判定」
- **省钱措施**：
...
- **管理员的处置动作**（都是复用已有功能，AI 不参与）：
  - 标记为「无问题」
  - 要求作者修改（发邮件说明）
  - 直接处置：关闭车帖、退回稿件、清空战队简介、要求改昵称、停用账号
...
| 字段 | 规则 |
|---|---|
| 游戏 ID | 必选，从自己的游戏 ID 里选一个，会公开显示在车帖上 |
| 模式 | 必选，从启用的游戏模式里选 |
| 缺的位置 | 必选，坦克、输出、支援至少勾一个 |
| 开车时间 | 必填，默认当前时间；范围是当前时间前 10 分钟到 7 天后（**默认**） |
| 备注 | 可选，最多 200 字 |

- 需要 `lfg_post` 权限，并且至少有 1 个游戏 ID
- 每人同时最多 3 个未过期的车帖（后台可配置）
- 过期时间在创建和修改时自动计算：开车时间 + 2 小时（后台可配置）
...
|---|---|---|
| 招人中 `open` | 默认状态 | |
| 已满 `full` | 车主标记人满了，仍然显示在列表里 | 车主可以在「招人中」和「已满」之间切换 |
| 已关闭 `closed` | 不再显示 | 车主或管理员关闭，关闭后不能重新打开 |
| 已过期 | 不是单独的状态，当前时间超过过期时间就不显示 | — |

- 车主可以修改车帖的所有字段，修改开车时间会重新计算过期时间
- 管理员可以在后台关闭违规车帖

### 6.3 列表

- 需要登录才能查看（**默认**）
- 只显示状态为「招人中」或「已满」、没有过期、车主账号未停用的车帖
- 排序：按开车时间从早到晚，已满的排在后面
- 筛选：模式（单选）、缺的位置（多选，满足任意一个即可）、只看招人中
- 每 30 秒自动刷新列表
- 每张车帖卡片显示：模式、开车时间（相对时间，比如「20 分钟后」）、缺的位置图标、游戏 ID（带复制按钮）、车主昵称、备注、状态
...
erDiagram
    ...
    User ||--o{ LfgPost : "发车"
    GameAccount ||--o{ LfgPost : "使用"
    GameMode ||--o{ LfgPost : "模式"
...
| lfg_max_active_posts | 3 | 每人最多几个未过期车帖 |
| lfg_expire_hours | 2 | 车帖在开车时间之后多久过期 |
...
| name | 模式名称 |
| sort_order | 排序 |
| is_active | 停用后发车时不能选，已有车帖不受影响 |

初始数据：快速游戏、竞技（角色限定）、竞技（自由职责）、角斗领域、自定义。
...
| weight | smallint | 必须是所选字体已有的、处理完成的字重 |
...
| 模型 | 字段 |
|---|---|
| HomePage | 置顶文章（通过子表 `HomePagePinnedArticle` 维护，可排序，最多 3 篇） |
...
|---|---|---|
| owner | FK → User | 车主 |
| game_account | FK → GameAccount，级联删除 | 公开显示的游戏 ID |
| mode | FK → GameMode | |
| role_tank / role_damage / role_support | bool | 缺的位置 |
| start_at | datetime | 开车时间 |
| expires_at | datetime | 过期时间，保存时自动计算 |
| status | 枚举 | `open` / `full` / `closed` |
| note | varchar(200) | 备注 |

- 索引：(expires_at, status)
- 过期靠查询条件过滤，不需要定时任务
...
| 字段 | 类型 | 说明 |
|---|---|---|
| target_type | varchar(32) | 内容类型：`nickname` / `team_name` / `team_description` / `lfg_note` / `application_message` / `article` / `tournament_description` / `scrim_description` / `page` / `image` |
...
视觉风格还没有确定（第 19 章），但从第一行模板开始就遵守以下规则，保证以后换风格不用改模板：

- **模板里只使用 daisyUI 的语义颜色**，比如 `btn-primary`、`bg-base-100`、`text-base-content`、`badge-success`，**不直接写** `bg-orange-500` 这类具体颜色
- 开发阶段先用 daisyUI 的内置主题占位；风格确定后，只在 CSS 入口文件里定义自定义主题
- 深浅色切换也通过 daisyUI 主题实现，和视觉风格一起决定
- **字体同样不写死在模板里**：全部通过 CSS 变量引用，由后台的排版设置控制（13.12 节）
- 不使用暴雪官方的 logo、英雄立绘等素材，除非确认获得授权

### 13.3 全站布局

- **顶部导航栏**：站点名称；首页、资讯、赛事、组队大厅、战队、内战；右侧为「投稿」按钮和账号区域
- **账号区域**：未登录时显示「登录」「注册」；登录后显示昵称，下拉菜单包括个人中心、管理后台（有后台权限时）、退出登录
- **手机端**：导航折叠进侧边抽屉菜单（daisyUI 的 drawer 组件）
- **页脚**：关于我们、用户协议、隐私政策
- **操作提示**：页面右上角的提示条，由 Django 消息框架和 HTMX 的 `HX-Trigger` 共同驱动；在预渲染页面上通过页面状态片段显示（13.13.3 节）
...
| `/me/security/` | 修改密码、修改邮箱、两步验证 | 已登录 |
...
- 左侧（手机端为顶部标签）菜单：基本资料、游戏 ID 与段位、联系方式、我的战队、我的报名、账号安全
...
├── components/            可复用组件
│   ├── form_field.html    表单字段：标签、输入框、帮助文字、错误提示
│   ├── rank_badge.html    段位徽章
│   ├── role_icons.html    位置图标
│   ├── status_badge.html  各类状态标签
│   ├── pagination.html    分页
...
def lfg_list(request):
    posts = LfgPost.objects.visible().filter_by(request.GET)
    template = "lfg/list.html#post_list" if request.htmx else "lfg/list.html"
    return render(request, template, {"posts": posts})
```

片段在 `lfg/list.html` 里用 `{% partialdef post_list %}` 定义（Django 6 内置功能），整页和片段共用一份模板。
...
| 位置 | 交互 | 实现 |
|---|---|---|
| 组队大厅 | 修改筛选条件时只刷新列表；每 30 秒自动刷新 | 列表容器 `hx-get`，触发条件为筛选变化和 `every 30s` |
...
| 车帖卡片 | 一键复制游戏 ID | Alpine.js + 浏览器剪贴板接口 |
...
- Tailwind CSS + daisyUI 通过 django-tailwind-cli 编译（设置 `TAILWIND_CLI_USE_DAISY_UI = True`），不需要 Node
...
- 静态文件使用 WhiteNoise 的存储后端生成带内容哈希的文件名和预压缩版本；生产环境由 **Caddy 直接提供** `/static/`，设置长期缓存，不经过 Django
- 静态资源放在持久化的 `static` 数据卷里，升级时只新增文件、不删除旧版本的文件，保证旧的预渲染页面引用的资源仍然存在（旧文件 30 天后清理，16.5 节）
- 用户上传的文件由 Caddy 直接提供；文件名带哈希的（字体分片、图片缩略图）设置长期缓存
...
2. **切片**：把字体包含的字符分成若干片，每片单独生成一个 WOFF2 文件
   - 英文字母、数字、常用标点单独一片
   - 常用汉字按使用频率排序，每片约 200 字（**默认**）
   - 其余字符按 Unicode 区段切成较大的片
3. 为每一片生成带 `unicode-range` 的 `@font-face` 规则。浏览器只下载页面上实际出现的字符所在的分片
4. 分片文件名带内容哈希，设置长期缓存
5. 从 Google Fonts 下载的字体已经由 Google 切好片，直接下载全部分片、改写为本站地址，不再重新切片

- 处理一个中文字重可能需要几分钟、占用几百 MB 内存，所以任务队列同一时间只处理一个字体
...
- 保存排版设置时，生成一个 `fonts.<内容哈希>.css` 文件，包含：
  - 被使用的字体和字重对应的全部 `@font-face` 规则（没有被使用的字体不写入）
...
| 类型 | 页面 | 做法 |
|---|---|---|
| 预渲染 | 首页、文章列表、文章详情、普通页面、赛事列表、赛事详情、战队列表、战队主页、内战活动列表、内战活动详情 | 生成静态 HTML 文件；个人相关区域留出占位，页面加载后填充 |
| 静态外壳 | 组队大厅 | 导航、筛选栏等页面框架预渲染；车帖列表本来就通过 HTMX 加载，登录检查在列表请求里完成 |
| 实时渲染 | 个人中心、报名页、报名详情、战队管理、创建战队、申请入队、发车和编辑车帖、账号相关页面、投稿入口 | 每次由 Django 渲染 |
...
| 车帖数量 | 首页的组队大厅入口 | 已登录用户看到当前招人中的车帖数量；未登录访客看到「登录后查看」 |
...
- 页面在 `<head>` 里同步加载一个很小的本站脚本（不到 1KB，长期缓存），检查两个不含任何身份信息的提示 Cookie：
  ...
  - `ow_flash=1`：有待显示的操作提示时设置，提示显示后删除
- 两个 Cookie 都没有时，脚本什么也不做。**未登录访客浏览公开页面，完全不经过 Django**

**已登录用户只发一次请求**

- 脚本先给页面加上「已登录」的样式，占位区域显示骨架，避免先闪一下「登录」按钮
- 然后向 `/_fragments/state/` 发一个请求，带上本页的占位区域列表；Django 在一个响应里返回所有区域的内容，HTMX 用带外交换（OOB swap）分别填进各个占位区域
...
| 战队创建、修改、成员变动、招募状态变化、解散 | 战队主页、战队列表 |
| 内战活动创建、修改、发布、取消 | 活动详情、活动列表、首页 |
| 内战报名、修改报名、取消报名 | 活动详情（报名统计和名单） |
| 用户修改昵称 | 该用户所在的战队主页、报名的内战活动详情、署名的文章 |
| 文章分类、游戏模式修改 | 相关列表页、组队大厅外壳 |
...
- **`/robots.txt`**：禁止抓取 `/admin/`、`/api/`、`/me/`、`/accounts/`、`/_fragments/`
...
- 所有错误页面都**不依赖数据库**，在构建镜像时生成为静态文件
- 维护页面由 Caddy 在后端返回 502、503、504 或连接不上时直接提供
- 错误页面使用全站样式，但不包含个人相关区域
...
| 字体库 | 上传字体、从 Google Fonts 或网址下载、预览、下载字体文件、查看处理进度、重新处理、删除 | `ModelViewSet` + 自定义上传和下载视图 |
| 排版设置 | 9 个区域的字体、字重、字号、行高、字间距；实时预览；保存后生成字体样式表 | 自定义设置页面 + Alpine.js 预览 |
...
| 内容安全策略 | 使用 Django 6 内置的 CSP 支持。前台禁止内联脚本和 `eval`（使用 Alpine.js 的 CSP 兼容版本、关闭 HTMX 的 `allowEval`）；Wagtail 后台路径按需要单独放宽 |
| 密码 | Argon2 哈希；Django 密码强度校验 |
| 暴力破解 | allauth 限流：登录、注册、验证码、找回密码 |
| 业务操作限流 | 发车每人每小时 10 次、申请入队每人每天 20 次、创建战队每人每天 3 次、页面状态片段每个 IP 每分钟 120 次（**默认**）；超出时显示 429 页面或提示稍后再试 |
...
| 日志 | 内容 | 保留时间 |
|---|---|---|
| 访问日志 | Caddy 记录的请求：时间、IP、路径、状态码、耗时 | 14 天 |
| 应用日志 | 请求、错误、警告，输出到容器标准输出 | 按 Docker 日志轮转，保留 14 天 |
...
| `static` | 带哈希的静态资源，保留最近 30 天内的旧版本文件 | `web` 写入，`proxy` 读取 |
...
3. 同步一份到服务器以外的位置，比如对象存储（**默认**）。备份里有用户邮箱和联系方式，**上传前先加密**（比如用 age 加密文件）
...
- 服务器买下来之后、正式上线之前，分别在**校园网、电信、联通、移动的手机网络**下测试首页和组队大厅的打开速度，同时测试微信和 QQ 内置浏览器
...
- `main` 分支受保护，只能通过合并请求合入
- 每个功能一个分支，合并请求至少有 1 人审阅
- 提交信息说清楚改了什么、为什么改

### 17.6 数据库迁移

- 迁移文件和对应的模型修改在同一个合并请求里提交
- 已经合并到 `main` 的迁移文件不再修改，需要调整就新增迁移
- 初始化数据（用户组、权限、游戏模式等）由 `init_site` 命令负责，不写进迁移
...
| **M3 战队与组队大厅** | 战队创建、申请、审批、成员管理、解散；车帖发布、筛选、自动刷新、过期；把昵称、队名、简介、车帖备注、入队留言接入 AI 审核 | 战队和组队大厅的完整流程可用；手机和微信内置浏览器中可正常使用 |
...
| 3 | 后台管理角色是否强制两步验证 | 超级管理员强制，其他管理角色建议开启 | M1 开始前 |
| 4 | 投稿的初始分类有哪些 | 公告（不开放投稿）、赛事通知（不开放投稿）、攻略、战报、心得 | M2 开始前 |
| 5 | 视频嵌入是否需要支持 B 站 | 需要，国内玩家的视频主要在 B 站 | M2 开始前 |
| 6 | 用户注销账号、查阅和导出个人信息怎么处理 | 注销后匿名化：邮箱替换为无效地址、昵称改为「已注销用户」、删除游戏 ID 和联系方式、退出所有战队，历史报名快照保留；个人中心提供「导出我的个人信息」（JSON 文件） | M7 开始前 |
| 7 | 用户协议和隐私政策的文本 | 由社团负责人起草，写明个人信息存储在境外；必要时请学校相关部门把关 | M7 开始前 |
| 8 | 视觉风格和主题色 | 开发阶段先用 daisyUI 内置主题占位 | M7 开始前 |
| 9 | 是否支持深浅色切换 | 和视觉风格一起决定 | M7 开始前 |
...
| 12 | 是否开启「高风险内容暂缓公开」 | 默认关闭，保持「只有管理员处置」；如果担心违规内容在管理员处理前被看到，可以开启（5.5.5 节） | M3 开始前 |
...
| 功能标识 | `lfg_post` 发车、`team_create` 创建战队、`team_apply` 申请入队、`tournament_register` 报名赛事、`scrim_signup` 报名内战、`article_submit` 投稿 |
...
| 送审内容类型 `ModerationItem.target_type` | `nickname` 昵称、`team_name` 队名、`team_description` 战队简介、`lfg_note` 车帖备注、`application_message` 入队留言、`article` 文章或稿件、`tournament_description` 赛事说明、`scrim_description` 内战说明、`page` 普通页面、`image` 图片 |
...
| 队名长度 | 2 到 16 个字符 | 配置 |
| 战队简介 | 最多 500 字 | 配置 |
| 入队留言、拒绝原因、车帖备注 | 最多 200 字 | 配置 |
| 审核备注 | 最多 300 字 | 配置 |
| 图片上传 | JPG、PNG、WebP，不超过 5MB | 配置 |
| 每人最多未过期车帖 | 3 个 | 后台 |
| 车帖过期时间 | 开车时间后 2 小时 | 后台 |
| 开车时间范围 | 当前时间前 10 分钟到 7 天后 | 配置 |
| 组队大厅自动刷新 | 30 秒 | 配置 |
| 首页近期内战范围 | 未来 7 天 | 配置 |
| 文章列表每页 | 12 篇 | 配置 |
| 内战提醒时间 | 开始前 2 小时 | 后台 |
...
| 前台业务操作限流 | 发车每人每小时 10 次；申请入队每人每天 20 次；创建战队每人每天 3 次；页面状态片段每个 IP 每分钟 120 次 | 配置 |
