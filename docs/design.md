...
版式按 13.2 节的设计体系，从上到下（v2.0 重做，不再照交大官网）：

| 区块 | 内容来源 |
|---|---|
| 头条（刊物） | 左边**焦点图**（`c-feature`）：后台在首页里维护「焦点图」：图片、标题、链接，最多 6 张；一张都没有时，取最近 5 篇带封面的文章；再没有就只显示右边一栏。每 5 秒换一张（**默认**），下面是编号索引和进度红线。右边**近期**：正在报名的赛事（按截止时间）和未来 7 天内的内战（按开始时间），合起来最多 4 条（**默认**），写成紧凑的票根：日期、「截止」或「开始」、标题、状态 |
| 01 资讯 | 置顶文章排在最前，后面接最新文章，共 6 篇（**默认**），写成列表行：日期、分类、标题、摘要，有封面的带缩略图。侧栏是全部分类和投稿入口。置顶文章在后台指定，最多 3 篇 |
| 02 赛事与内战（赛场，深色） | 上面是**日程带**：从今天起 14 天（**默认**），每天一格，有公开或已结束的内战、有赛事开赛的日子标出数量；下面左边是正在报名和即将开放报名的赛事（票根，最多 3 张，带已通过的队数），右边是日程带里还没开始的内战（紧凑票根，带名额格）。点有活动的一格去那天的第一场 |
| 03 战队 | 最新成立的 6 支战队（**默认**），图块墙：队标、队名、人数、招募状态 |
| 04 参与 | 四个入口，每个一句话说明：报名赛事、报名内战、组建或加入战队、我要投稿；旁边写成员总数，链接到成员展示 |
...
- 前台「投稿」入口在资讯栏目页的页头、首页（资讯区块旁和「参与」区块）、个人中心、页脚和手机菜单的「更多」里（v2.0 起主导航不再有「投稿」，13.3 节），链接到后台新建稿件页，栏目默认选中「开放投稿的第一个栏目」
...
**用户决定（2026-09-26）：主色仍然是上海交大的深红色，其余全部推倒重来。** 065 那套照交大官网做的风格（红色页头、米色、宋体、印章标志、日期方块、玫瑰色日历、拱形卡片、楷体引用）整体作废，**不沿用任何元素**。下面这套体系是看过游戏官网、电竞数据站、电竞俱乐部、高校和编辑类网站之后，按本站的用途重新推导的。

**本站的用途决定了风格**：访客大多从 QQ、微信群里点链接进来，用手机看一条公告、报一场内战、找一支战队；干部在电脑上发文章、排赛事。内容一半是「读」（资讯、文章、协议），一半是「报名和查名单」（赛事、内战、战队）。所以这套体系有两种声部，共用一套颜色、字体和组件。

#### 13.2.1 参考了什么、学什么、不学什么

| 参考 | 学 | 不学 |
|---|---|---|
| 守望先锋官网 | 组件词汇很少（主要、次要、文字三种按钮）；全站几乎直角；品牌色只给按钮和焦点；数值格式严格统一 | 首页四张一样的亮点卡片；彩色左边框的注释框 |
| Riot Games、英雄联盟 | 品牌红几乎只用在文字、线条和状态上，不大面积铺底；标题和正文两种声部 | 胶囊形按钮 |
| 无畏契约 | 浅色和深色区块交替，一页里有节奏；主要按钮有自己的形状 | 英文大写当主要语言；超窄的巨型字 |
| 明日方舟官网 | 中英双语的栏目标签；把真实的元数据当装饰（页码、日期写成 `2026 // 09 / 04`）；新闻是发丝线分隔的行，不是卡片；全站一种缓动 | 整屏翻页；藏起来的导航 |
| Counter-Strike 2 更新日志 | 朴素的日期列表；方括号小标题 `[ 地图 ]` | —— |
| 米哈游系官网 | 新闻按分类切换标签 | 整屏翻页、涂鸦贴纸风 |
| Liquipedia、HLTV、VLR | 赛程按天分组、时间单独一列；资料框（左键右值的表）；数据密但靠对齐和线条读得清 | 广告位、过密的侧栏 |
| 电竞俱乐部官网 | 队伍、队员用统一比例的方形图块成墙排列 | 商城、倒计时式的催促文案 |
| 高校官网、GOV.UK、编辑类网站 | 栏目头（眉标 + 大标题 + 导语）；正文限制行宽；「服务入口」写成一句话说明加箭头；页脚写全真实信息 | 首页大图配两个按钮的落地页套路 |

用户以前否掉过的：大图配两个按钮的横幅、2×2 一样的区块、彩色左边框的提示框、HeroUI 那种软圆角卡片（「不正式」「AI 味」）。这些在本体系里都不出现。

#### 13.2.2 原则

1. **红色是信号，不是底色**。交大红只用在四个地方：品牌标志、「当前位置」的红线、每一屏最多一个的主要操作、需要行动的状态（报名中、进行中）。大面积的底色只用中性色
2. **两种声部**：
   - **刊物**：资讯、文章、协议、成员展示、个人中心。浅底，排版为主，发丝线分隔
   - **赛场**：赛事、内战、战队。深色的页头或区块，大号数字，票根和名额格
   - 同一页里赛场声部只出现在页头或一个区块，不整页深色
3. **线而不框**：结构靠 1px 发丝线和网格，不靠卡片、阴影和圆角。全站直角
4. **数字是材料**：日期、时间、人数、段位、名次用专门的数字字体和等宽数字，格式全站统一（13.2.4）
5. **只印真的信息**：编号、日期、名额、页码都是真实数据。不写假的 HUD 字样、假坐标、装饰性英文
6. **中文为主**：英文只作很小的眉标（`NEWS`、`TOURNAMENTS`），不做主要语言
7. **手机优先**：点击区域不小于 44×44 像素；表格在手机上变成列表；导航收进抽屉
8. **状态不只靠颜色**：每个状态同时有文字和形状（13.2.6）

#### 13.2.3 颜色

所有颜色只在 `assets/css/input.css` 的 `@theme` 里定义一次，模板只用语义名（`bg-canvas`、`text-ink-2`、`border-line`）或组件类，**不写色值**。

| 令牌 | 色值 | 用途 |
|---|---|---|
| `red` | `#B2141A` | 交大红。标志、红线、主要按钮、「报名中」 |
| `red-deep` | `#861116` | 主要按钮按下、悬停；浅底上的红色文字（对比度 9.2） |
| `red-bright` | `#EA4E4B` | 深色区块上的红色文字和线（对比度 5.1） |
| `red-tint` | `#FBEAE9` | 浅底上选中的行、错误字段的底 |
| `canvas` | `#F6F5F4` | 页面底色 |
| `surface` | `#FFFFFF` | 输入框、表格、需要和底色分开的面 |
| `sunken` | `#EFEDEC` | 表头、代码、缺图时的占位 |
| `line` | `#DDD9D7` | 发丝线 |
| `line-strong` | `#B6B0AE` | 装饰性的强线：标签、位置块、名额格、提示框、下拉菜单的外框，票根的虚线（不靠它认出控件） |
| `control` | `#847C7A` | 输入框、下拉框、复选框、单选、选择块的边框（和白底 4.0、页面底色 3.7） |
| `ink` | `#1D1716` | 正文、标题 |
| `ink-2` | `#5C5452` | 次要文字（对比度 6.8） |
| `ink-3` | `#726A68` | 第三级文字：日期、副行、署名、序号、占位文字（对比度 4.8，凹陷底色上 4.5） |
| `night` | `#141010` | 深色：页头、页脚、赛场区块 |
| `night-2` | `#1F1A19` | 深色上的面：票根、输入框 |
| `night-line` | `#3A3231` | 深色上的线 |
| `night-ink` | `#F5F2F1` | 深色上的文字 |
| `night-ink-2` | `#ADA5A3` | 深色上的次要文字（对比度 7.8） |
| `night-ink-3` | `#8F8785` | 深色上的第三级文字（对比度 5.3，`night-2` 上 4.9） |
| `night-line-strong` | `#5A5150` | 深色上的 `line-strong`（装饰） |
| `night-control` | `#736A69` | 深色上的输入框边框（对比度 3.5，`night-2` 上 3.2） |
| `ok` | `#2F6F45` | 已通过、已加入 |
| `warn` | `#8F5B06` | 待审核、即将截止 |
| `info` | `#3F6789` | 即将开始、未开放 |
| `ok-bright` / `warn-bright` / `info-bright` | `#7CC093` / `#E0A84A` / `#8FB1CF` | 深色上的三种状态色（对比度都在 7.6 以上） |

- 中性色都带一点点红色的色相（OKLCH 色相 25°、彩度 0.01 以内），和交大红放在一起不发冷，但看起来仍然是灰和黑，不是米色
- **对比度**（WCAG 2.1 AA）：文字和它落在的底色之间至少 4.5:1（1.4.3）；表单控件的边框和相邻底色之间至少 3:1（1.4.11）。只起装饰作用的线（`line`、`line-strong`）不在此列。下表的组合由测试从 `input.css` 读出令牌逐对计算，改令牌时一起检查：

| 前景 | 底色 | 至少 |
|---|---|---|
| `ink`、`ink-2`、`ink-3`、`red`、`red-deep`、`ok`、`warn`、`info` | `canvas`、`surface`、`sunken`、`red-tint` | 4.5 |
| `night-ink`、`night-ink-2`、`night-ink-3`、`red-bright`、`ok-bright`、`warn-bright`、`info-bright` | `night`、`night-2` | 4.5 |
| `white` | `red`、`red-deep`（主要按钮和它的悬停） | 4.5 |
| `control` | `canvas`、`surface`、`sunken` | 3 |
| `night-control` | `night`、`night-2` | 3 |

- **单一主题**：浅色页面加深色的页头、页脚和赛场区块。不跟随系统深色模式，也不做切换（19.2 第 9 条）：页面是预渲染的，同一个文件给所有人

#### 13.2.4 字体、字号和数字格式

只用系统字体，不下载任何字体文件（13.10）；后台「排版设置」仍然可以换（13.12）。

| 用途 | 默认字体 | 说明 |
|---|---|---|
| 正文、标题、导航、按钮 | 苹方（苹果）、鸿蒙黑体、MiSans、微软雅黑（Windows）、思源黑体 / Noto Sans CJK（安卓和 Linux） | 标题用同一套字体加粗，靠字号和字重拉开层级 |
| 大号数字、眉标 | DIN Condensed、DIN Alternate（苹果系统自带）、Bahnschrift（Windows 10 起自带），都没有时用正文字体 | CSS 变量 `--font-figure`。后台给「数字与数据」设了字库里的字体时跟着换（13.12.4） |
| 行内数字 | 同正文，开启等宽数字 | 「数字与数据」区域，`font-numeric` |
| 游戏 ID、分队结果 | 系统等宽字体 | 「游戏 ID 与代码」区域，`font-code` |

字号（桌面；手机上标题缩到 85%，13.12.3）：

| 层级 | 字号 / 行高 | 用在 |
|---|---|---|
| 特大数字 | 3.5rem / 1 | 赛场页头的日期、404 这样的状态码 |
| 一级标题 | 2.25rem / 1.3，字重 700 | 页面标题 |
| 二级标题 | 1.5rem / 1.35，字重 700 | 区块标题、正文小节 |
| 三级标题 | 1.125rem / 1.45，字重 600 | 列表行标题、卡片标题 |
| 正文 | 1rem / 1.7 | 界面文字 |
| 文章正文 | 1.0625rem / 1.85 | `c-prose`，行宽不超过 40 个汉字 |
| 小字 | 0.875rem / 1.6 | 元信息、帮助文字 |
| 眉标 | 0.75rem，字距 0.14em，大写 | 英文栏目标签、表头 |
...
- **网格**：内容最宽 76rem（1216px），12 栏，栏距 24px；页面左右留白手机 16px、平板 24px、桌面 32px。常用分法：主栏 8 + 侧栏 4、正文 7 居中
- **间距**：以 4px 为单位。区块之间桌面 64px、手机 40px；组件内边距 16 / 20 / 24px
- **形状**：全部直角。例外只有两个：状态点（圆点），和单选框（圆的，免得一组单选看起来像可以多选）
- **线**：发丝线 1px `line`；强调线 2px `ink`（表头下面、栏目头下面）；红线 3px `red`（13.2.6）
- **阴影**：不用。下拉菜单和抽屉靠 1px 边框和底色分开
- **焦点**：2px 实线轮廓，偏移 2px；浅底用 `red`，深底用 `red-bright`
- **动效**：全站一条缓动 `cubic-bezier(0.2, 0.7, 0.2, 1)`；悬停 120ms，展开 200ms，焦点图切换 600ms。系统设了「减少动态效果」时全部关掉
...
| 元素 | 样子 | 只用在 |
|---|---|---|
| **红线** | 3px 交大红的横线或竖线 | 「你在这里」：当前导航项的下沿、当前筛选、个人中心当前菜单的左沿、焦点图当前一张的进度 |
| **切角** | 右下角 45° 切掉一个小三角（按钮 8px，票根和大图 14px） | 主要按钮、票根、首页焦点图、品牌标志。次要按钮和普通面板不切 |
| **编号** | `01`、`02` 两位数字，数字字体 | 首页区块、个人中心菜单、表单的分步小标题、焦点图的索引、成员名册的序号 |
| **名额格** | 一排小方块，实心是已报，空心是空位 | 内战报名人数、战队人数 / 上限。超过 24 格改成分段条 |
| **斜纹** | 45° 细斜线填充 | 「没有」：没有封面、没有队标、空的名额、禁用 |
| **票根** | 左边一栏大号日期，虚线分开，右边是标题和要点，右下切角 | 赛事和内战的卡片 |
...
| 即将开始、未开放 | 空心方块 | `info` |
| 已截止、已结束 | 实心方块 | `ink-3` |
| 已取消、已驳回、已撤回 | 斜杠 | `ink-3`（驳回用 `red-deep`） |

#### 13.2.7 组件

组件类以 `c-` 开头，布局类以 `l-` 开头，都写在 `input.css` 里；模板里其余的只用 Tailwind 的间距、排列类。**前台不再使用 daisyUI**（v2.0 起）。

| 组件 | 类名 | 要点 |
|---|---|---|
| 页头 | `c-masthead` | 深色条，高 64px：标志、主导航、搜索、账号区域；手机上是标志、搜索和菜单按钮 |
| 抽屉 | `c-drawer` | 手机导航，页头下面展开的深色整屏，导航项带编号；用 `<details>`，不靠脚本 |
| 页脚 | `c-footer` | 深色，四栏链接，底行写版权和「不是上海交通大学官方网站」 |
| 面包屑 | `c-crumbs` | 页面标题上方一行，`/` 分隔 |
| 栏目头 | `c-pagehead` | 眉标、一级标题、导语、元信息、操作。`--night` 变体用于赛场页 |
| 区块头 | `c-sectionhead` | 编号、二级标题、英文眉标、延伸到右边的发丝线、「全部 →」 |
| 按钮 | `c-btn` | `--primary`（红底、切角，每屏最多一个）、`--secondary`（墨色描边）、`--quiet`（文字加箭头）、`--danger`（红色描边）；`--sm`、`--block`；高至少 44px（`--sm` 36px，只用在表格行内） |
| 标签 | `c-tag` | 分类、规格。1px 描边的小方框，不填色 |
| 状态 | `c-status` | 形状 + 文字（13.2.6） |
| 段位 | `c-rank` | 大段名 + 小段数字；未定级显示「未定级」 |
| 位置 | `c-roles` | 坦克、输出、支援三个自绘图标加文字，不用官方素材 |
| 名额格 | `c-slots` | 13.2.6 |
| 数据 | `c-stat` | 眉标 + 大号数字 + 单位 |
| 资料表 | `c-facts` | 左键右值，发丝线分行；手机上仍然两列 |
| 表格 | `c-table` | 表头眉标、表头下 2px 墨线、行间发丝线、数字右对齐；手机上每行变成一段 |
| 列表行 | `c-rowlist` | 资讯、日程、搜索结果：日期列、标签、标题、摘要，可带缩略图 |
| 票根 | `c-ticket` | 13.2.6。浅底、深底两种 |
| 日程带 | `c-daystrip` | 首页的未来 14 天，一天一格，有活动的一格标红线和数量 |
| 焦点图 | `c-feature` | 首页头条：大图（切角）、标题、编号索引、进度红线；5 秒一张，可暂停 |
| 图块墙 | `c-tiles` | 战队：方形队标、队名、人数、招募状态；没有队标时斜纹底加队名首字 |
| 名片 | `c-person` | 成员：方形首字、昵称、职务、战队 |
| 头像 | `c-avatar` | 方形；用户没有头像，一律显示昵称首字 |
| 表单字段 | `c-field` | 标签在上，输入框 1px `control`、直角、高 44px；帮助文字、错误文字（带图标，字段底色变 `red-tint`） |
| 分步 | `c-step` | 表单里的一组字段，带编号小标题：`01 选择游戏 ID` |
| 提示 | `c-notice` | 整行，前面图标、后面文字，1px 边框；`--info`、`--warn`、`--error`、`--ok`。**不用彩色左边框** |
| 操作提示 | `c-toast` | 右上角，深色条，4 秒后淡出（13.3） |
| 空状态 | `c-empty` | 斜纹框里一句话和至多一个操作 |
| 面板 | `c-panel` | 一组必须圈起来的内容（游戏 ID、报名框、待审批的申请）：1px 线框、面底色，不加阴影和圆角。能用发丝线分隔的就不用面板 |
| 文字链接 | `c-link` | 正文和提示里的链接：下划线，悬停变红 |
| 分页 | `c-pager` | 上一页、`2 / 7`、下一页 |
| 标签条 | `c-tabs` | 下划线式，当前项红线；是链接，不是脚本切换 |
| 正文 | `c-prose` | 文章、协议、赛事说明的富文本：段落不缩进、段间空一行；引用是上下两条发丝线夹着的大一号文字，出处右对齐 |
| 评论 | `c-comments` | 方形首字头像；回复串缩进并用发丝线连起来；置顶的一条上沿是 2px 墨线加「置顶」标签（红线只表示当前位置） |
| 小操作 | `c-act` | 评论里的赞、回复、编辑、删除、置顶、隐藏：图标加文字，不画边框，点击区域仍是 44px 高 |
...
| 骨架 | 结构 | 用在 |
|---|---|---|
| **刊物列表** | 栏目头（可带一个次要操作，比如「我要投稿」）→ 标签条 → 列表行 → 分页 | 资讯、搜索结果 |
| **刊物正文** | 面包屑 → 标题和元信息 → 头图 → 正文（7 栏居中）→ 文末信息 → 评论 | 文章、协议、关于 |
| **赛场列表** | 深色栏目头（带统计数字）→ 按阶段分组：进行中的用票根，结束的用紧凑表格 | 赛事、内战 |
| **赛场详情** | 深色栏目头（状态、标题、关键事实条、报名入口）→ 主栏说明和名单 + 侧栏资料表 | 赛事、内战、战队主页 |
| **名册** | 栏目头 → 分组区块（名片网格）→ 全部成员（编号名册） | 成员展示 |
| **表单** | 栏目头 → 分步字段组 → 底部操作条（手机上贴底） | 报名、创建战队、申请入队、登录注册 |
| **个人中心** | 左侧编号菜单（手机上是横向滚动的标签）+ 右侧内容 | `/me/` 下的页面 |

各页用哪些区块：首页见 5.2 节；赛事 8.2；内战 9.2；战队 7.2、7.6；成员 6.3；个人中心 13.5。

#### 13.2.9 实现规则

- 模板只用语义颜色和组件类，不写色值、不写字体名（13.12）；不写行内样式（内容安全策略 `style-src 'self'`，已有测试扫描）
- 新组件先写进本节的表和样张页，再在页面里用
- 图标全部自绘（`components/icon.html`），24×24、线宽 1.75、直角端点；不引入图标库，不用暴雪的 logo 和英雄立绘
- 站点标志是一个红色切角方块，旁边是「交大守望先锋」和一行 `SJTU OVERWATCH COMMUNITY`；**不用交大的校徽、书法校名和照片**，页脚写明不是官方网站
...
- **页头**（`c-masthead`，深色，高 64px）：左边站点标志；中间主导航：首页、资讯、赛事、内战、战队、成员，当前栏目下沿一条红线；右边搜索框和账号区域
- **账号区域**：未登录时显示「登录」「注册」（注册是次要按钮样式）；登录后显示方形首字头像和昵称，点开是个人中心、我的报名、退出（`<details>` 下拉，不靠脚本）
- **手机端**：页头只留标志、「登录」（或头像）、搜索按钮（去 `/search/`）和菜单按钮；菜单是在页头下面展开的深色整屏（`<details>`，不靠脚本），导航项带编号，下面是搜索框和更多链接
- **页头上方**：测试环境横幅、微信内置浏览器提示（16.9、16.10）各占一整行
- **页脚**（`c-footer`，深色）：左边标志和一段说明（学生社团自办网站，不是上海交通大学官方网站，与游戏开发商、运营商无关）；右边四栏链接：社区（资讯、成员、战队）、参与（赛事、内战、投稿）、账号（个人中心、登录、注册）、关于（关于我们、用户协议、隐私政策）；最下面一行版权
- **操作提示**：页面右上角（手机上是顶部）的深色提示条（`c-toast`），由 Django 消息框架和 HTMX 的 `HX-Trigger` 共同驱动；在预渲染页面上通过页面状态片段显示（13.13.3 节）
...
- 左侧（手机端为顶部横向滚动的标签）菜单，带编号 `01`–`07`，当前项左沿一条红线（手机上是下沿）：基本资料、游戏 ID 与段位、联系方式、我的战队、我的报名、我的内战、账号安全
...
- 大号数字和眉标用 `--font-figure`（13.2.4）：默认是系统自带的 DIN 类字体，接着是 `--font-numeric`。「数字与数据」区域选了字库里的字体时，生成的样式表同时写 `--font-figure`，让大号数字也换成那个字体
...
| 报名变为「已通过」，或者从「已通过」变为其他状态 | 赛事详情（已通过的战队列表）、战队主页（参赛记录） |
...
- **兜底**：每天凌晨全量重新生成一次；访客请求到还没有静态文件的公开页面时，Django 正常渲染，同时排一个生成任务
...
| 8 | ~~视觉风格和主题色~~ | **已决定（2026-09-26，推翻 2026-09-19 的「照交大官网」）**：主色仍是交大红，其余按 13.2 节的设计体系重做 | 已确定 |
| 9 | ~~是否支持深浅色切换~~ | **已决定**：不做切换、不跟随系统。2026-09-26 起是单一主题：浅色页面加深色的页头、页脚和赛场区块（13.2.3） | 已确定 |
...
| 首页近期内战范围 | 未来 7 天 | 配置 |
| 首页「近期」 | 最多 4 条（正在报名的赛事 + 未来 7 天的内战，按日期合并） | 配置 |
| 首页资讯 | 6 篇，置顶的在前 | 配置 |
| 首页日程带 | 从今天起 14 天 | 配置 |
| 首页赛场区块的赛事 | 最多 3 个（报名中在前，即将开放在后） | 配置 |
| 首页战队 | 最新成立的 6 支 | 配置 |
| 文章页同栏目最新 | 4 篇 | 配置 |
| 文章列表每页 | 12 篇 | 配置 |
| 内战提醒时间 | 开始前 2 小时 | 后台 |
