...
版式按 13.2 节的设计体系（v4.0），从上到下：

| 区块 | 内容来源 |
|---|---|
| 首屏（`c-hero`） | 全宽的首屏图（全站设置「首屏图片」，13.2.5），桌面高 `min(72vh, 640px)`、手机 440px；图的下半部压一层 `scrim`。左下是展示字号的标题「上海交通大学守望先锋社区」、一句简介、主要按钮「加入社区」（去注册页；首页是预渲染的，对所有人都一样）和白色描边按钮「加入 QQ 群」（全站设置的 QQ 群链接，没填就不显示）。没上传首屏图时是深色底加白色校徽（13.2.5） |
| 数字条（`c-stats`） | 紧贴首屏下面：注册成员（成员展示的口径：未停用、验证过邮箱）、战队（未解散）、累计内战（已结束的场数）、社区已成立「N 年 M 天」（按全站设置的社区成立日期算，没填不显示） |
| 近期 | 区块头「近期」和「全部内战 →」。**正在报名的赛事**（按截止时间最近的一场）写成大图卡（`c-feature`）：封面、「报名中」、标题、「报名截止 MM.DD HH:MM · 已通过 N 队」，整张卡链接到赛事详情；没有正在报名的赛事就不显示这张卡。**未来 7 天内的内战**写成列表行，最多 4 条（**默认**）：日期块、标题、「周五 19:30 · 已报 8 / 10」和进度条、状态（报名中 / 报名已截止）。两样都没有时写「最近没有安排」和去赛事、内战列表的链接 |
| 资讯与公告 | 左边「资讯」：置顶和最新文章共 4 篇（**默认**），图片卡两列两行；右边「公告」：「公告」和「赛事通知」两个分类最新的 5 篇（**默认**），列表行（日期块 + 标题）。各有「全部 →」 |
| 战队 | 最新成立的 6 支（**默认**）：队标、队名、「N 人」、招募中的写「招募中」；「全部 N 支战队 →」 |

v3.0 的关键数字条递增、近期安排卡、快捷入口磁贴、活动统计卡去掉：入口靠主导航和各区块的「全部 →」，数字并进数字条。

**文章页**按 13.2.7 节的「刊物正文」：44rem 正文栏居中，面包屑、分类、标题、作者和发布日期、封面（有才显示）、正文、关联赛事的图片卡、评论区；正文后面一行列出同栏目最新 3 篇（**默认**）的图片卡。摘要只在列表和分享信息里用，文章页不显示。正文里的「引用」块左侧一条竖线，出处写在下面「——出处」。
...
- 版式按 13.2.7 节的「名册」：栏目头下面是分组目录（点击跳到对应区块）和「全部成员」
- 按分组的排序，每个分组一个区块：分组名、简介、成员卡片（昵称、职务、战队）；组内按排序，再按昵称
- 最后是「全部成员」：所有已加入的用户，按加入时间先后排列并编号（`001` 起），每人一张小卡：首字头像、昵称、所在分组（v3.0）
- 还没有任何分组时，只显示「全部成员」
...
| founded_on | | 社区成立日期（可空）。首页数字条「社区已成立 N 年 M 天」按它算，空着不显示这一项（5.2） |
| qq_group_url | | QQ 群链接（可空，只能是 `https://` 开头）。首页首屏的「加入 QQ 群」按钮链接到这里，空着不显示（5.2） |
| hero_image | | 首页首屏图片（可空，外键到 Wagtail 图片）。建议 2400×1000 以上；空着时首屏是深色底加校徽（5.2、13.2.5） |
...
**v4.0（2026-09-26）**。用户看过 v2.0、v3.0 和当天十余版样稿（Material 3、液态玻璃、thyuu、守望先锋官网、Material You 等）后决定：**之前所有前台设计作废，从零重做，由 Claude 直接定一套**；可以用游戏官方素材（13.2.5）；深浅色跟随系统。

用户在这个过程里明确说过的要求，是这一版的约束：

- 深红是主色，其他颜色合理搭配，不能整片都是暖色
- 以内容为导向
- 不要画蛇添足的说明小字和小点
- 不要「AI 味」：渐变光斑、磨砂玻璃底、背景混色、水印光带、英文眉标、数字递增这类装饰都不要

**本站的用途**：访客大多从 QQ、微信群点链接进来，用手机看一条公告、报一场内战、找一支战队；干部在电脑上发文章、排赛事。页面要像一个认真运营的游戏社区官网：图和内容在前，一眼看到最近有什么比赛、有什么新闻。

#### 13.2.1 原则

1. **图和内容先行**：首页首屏、赛事、文章、战队先给图片（后台上传的官方素材、封面、队标），文字压在图上或紧跟在图下。没有图时用排版顶替，不画装饰图案
2. **中性底色，一个强调色**：页面是冷中性的白和灰（深色模式是近黑和深灰）；**深红只用在主要按钮、当前导航项、链接和「报名中」**，不做大面积底色
3. **不做装饰**：不用渐变色块（压在图片上保证文字可读的那层暗色渐变除外）、磨砂玻璃、水印、光带、点阵、眉标、数字递增、滚动出现的动画。阴影只给浮在页面上的东西（菜单、操作提示）
4. **字不小于 14px**，每段字都要有信息。状态、日期、人数用正常大小的字写清楚，不靠小圆点、小徽标
5. **深浅两套，跟随系统**（`prefers-color-scheme`），不做切换按钮。预渲染的静态页同一个文件对两种模式都成立
6. **手机优先**：点击区域不小于 44×44 像素；表格在手机上变成列表
7. **状态靠文字**：状态标签是一个词加底色，不加形状

#### 13.2.2 颜色

所有颜色只在 `assets/css/input.css` 里定义一次：浅色值写在 `@theme`，深色值写在 `@media (prefers-color-scheme: dark)` 里的 `:root` 覆盖同名变量。模板只用语义类（`bg-bg`、`text-fg-2`、`border-line`）或组件类，**不写色值**。

| 令牌 | 浅色 | 深色 | 用途 |
|---|---|---|---|
| `bg` | `#F5F6F8` | `#0E1014` | 页面底色 |
| `surface` | `#FFFFFF` | `#171A20` | 卡片、页头、页脚、输入框 |
| `surface-2` | `#EEF0F3` | `#20242B` | 表头、悬停、没有图时的占位块、日期块 |
| `line` | `#E1E4E8` | `#2C313A` | 卡片描边、分隔线（装饰性） |
| `control` | `#767D88` | `#6B7380` | 输入框、复选框的边框（至少 3:1） |
| `fg` | `#111318` | `#ECEEF1` | 正文、标题 |
| `fg-2` | `#4A505B` | `#B4BAC3` | 次要文字：摘要、说明 |
| `fg-3` | `#5F6671` | `#8F97A3` | 日期、署名、占位文字 |
| `primary` | `#A4161A` | `#C4262D` | 主要按钮的底、当前导航项的下划线、进度条、站点标志 |
| `primary-hover` | `#861116` | `#D9363D` | 主要按钮悬停 |
| `primary-text` | `#A4161A` | `#FF7A7E` | 红色文字：链接、「报名中」、标题里的关键词 |
| `primary-soft` / `on-primary-soft` | `#FCE8E8` / `#7A0F13` | `#3A1A1D` / `#FFB3B5` | 「报名中」「招募中」的标签，当前筛选项 |
| `ok` / `ok-soft` | `#1F7A45` / `#E3F3E8` | `#5FD18B` / `#15291D` | 已通过、已加入 |
| `warn` / `warn-soft` | `#8A5A00` / `#FBF0D9` | `#F0B44C` / `#2E2410` | 待审核、即将截止 |
| `info` / `info-soft` | `#2B5F96` / `#E5EEF8` | `#7FB2EE` / `#162435` | 即将开始、未开放；和深红搭配的冷色 |
| `scrim` | 黑色 0 → 70% | 同左 | 压在图片下半部的暗色渐变，只用在图片上 |

- **对比度**（WCAG 2.1 AA）：文字和底色至少 4.5:1，控件边框至少 3:1。两种模式都要满足，由测试从 `input.css` 读出令牌逐对计算：`fg`、`fg-2`、`fg-3`、`primary-text`、`ok`、`warn`、`info` 在 `bg`、`surface`、`surface-2` 上；白字在 `primary` 上；各 `on-…-soft` 和状态色在自己的 `-soft` 上；`control` 在 `bg`、`surface` 上
- 半透明色只能由令牌用 `color-mix()` 混出来，不在令牌以外写色值，有测试扫描

#### 13.2.3 字体、字号和数字格式

只用系统字体，不下载字体文件；后台「排版设置」仍然可以换（13.12）。

| 用途 | 默认字体 |
|---|---|
| 正文、标题、导航、按钮 | 苹方、鸿蒙黑体、MiSans、微软雅黑、思源黑体 / Noto Sans CJK |
| 大号数字 | 同正文，加粗，等宽数字（`--font-figure`） |
| 游戏 ID、分队结果 | 系统等宽字体（`font-code`） |

| 层级 | 字号 / 行高 | 用在 |
|---|---|---|
| 展示 | 3rem / 1.15，字重 700（手机 2rem） | 首页首屏标题、赛事横幅标题 |
| 一级标题 | 2.25rem / 1.25，字重 700（手机 1.75rem） | 页面标题、文章标题 |
| 二级标题 | 1.5rem / 1.35，字重 700 | 区块标题 |
| 三级标题 | 1.125rem / 1.45，字重 600 | 卡片标题、列表行标题 |
| 正文 | 1rem / 1.7 | 界面文字 |
| 文章正文 | 1.0625rem / 1.85 | `c-prose`，行宽不超过 40 个汉字 |
| 小字 | 0.875rem / 1.6 | 日期、分类、状态、元信息。**全站最小字号** |
...
- **宽度**：内容最宽 76rem（1216px），左右留白手机 16px、平板 24px、桌面 32px；文章正文栏 44rem 居中
- **间距**：以 4px 为单位；区块之间桌面 64px、手机 40px；卡片内边距 16 / 20px
- **圆角**：4px（标签、进度条）、8px（卡片、图片、按钮、输入框、菜单）、圆形（头像、队标）。不用胶囊形状的大按钮
- **描边与阴影**：静止的卡片用 1px `line` 描边，不加阴影；菜单和操作提示用一层阴影
- **焦点**：2px 实线轮廓，偏移 2px，颜色 `primary`
- **动效**：只有颜色、描边的切换（150ms）和卡片悬停时图片放大到 1.03 倍（300ms，`cubic-bezier(0.2, 0, 0, 1)`）。系统设了「减少动态效果」时关掉。不做进场动画、滚动出现、视差、数字递增

#### 13.2.5 图片

| 位置 | 来源 | 比例 | 没有图时 |
|---|---|---|---|
| 首页首屏 | 全站设置「首屏图片」（`SiteSettings.hero_image`，12.4.1），建议 2400×1000 以上 | 铺满，按中心裁切 | 深色底（深色模式的 `bg`）上一枚白色校徽，透明度 0.08，靠右 |
| 赛事卡片、赛事横幅 | 赛事的封面（`Tournament.cover`） | 16:9 | `surface-2` 的块，中间写赛事名 |
| 文章卡片、文章头图 | 文章封面 | 16:9 | `surface-2` 的块，中间写分类名 |
| 战队 | 队标 | 1:1，圆形 | 圆形 `surface-2` 上写队名首字 |

- **官方素材**：用户决定（2026-09-26）可以用守望先锋的官方图片（英雄原画、地图、赛季主视觉）。图片由社团在后台上传，**仓库不收录**任何暴雪素材（仓库是公开的）；是否符合暴雪的同人内容政策、怎么署名由社团负责。页脚写明「游戏图片版权归暴雪娱乐所有」
- **图上的字**：只写在图片下半部，底下压一层 `scrim`（从透明到 70% 黑的线性渐变），字用白色。不在图上放按钮以外的控件
- 所有图片都用 Wagtail 的缩略图（带宽高，`loading="lazy"`，首屏图除外）
...
| 组件 | 类名 | 要点 |
|---|---|---|
| 页头 | `c-masthead` | `surface` 底，底边 1px `line`，高 64px，贴在顶部不动：标志、主导航、搜索按钮、账号区域（13.3） |
| 主导航 | `c-nav` | 文字链接，当前项文字 `fg`、下方 2px `primary` 下划线，其余 `fg-2` |
| 抽屉 | `c-drawer` | 手机导航，页头下面展开的面板；用 `<details>`，不靠脚本 |
| 页脚 | `c-footer` | `surface` 底，顶边 1px `line`：标志和一句说明、四栏链接、底行版权和声明 |
| 面包屑 | `c-crumbs` | 页面标题上方一行，`/` 分隔 |
| 栏目头 | `c-pagehead` | 一级标题、一句导语、右侧至多一个操作 |
| 区块头 | `c-sectionhead` | 二级标题，右边「全部 →」文字链接 |
| 按钮 | `c-btn` | 高 44px（`--sm` 36px）、8px 圆角：`--primary` 深红实心白字（每屏最多一个）、`--secondary` 描边、`--ghost` 文字、`--light` 图片上的白色描边、`--danger`；`--block` 占满一行 |
| 标签 | `c-tag` | 分类、规格：4px 圆角，`surface-2` 底，`fg-2` 字 |
| 状态 | `c-status` | 一个词加底色：报名中、招募中用 `primary-soft`；通过用 `ok-soft`；待审核用 `warn-soft`；即将开始用 `info-soft`；截止、结束、取消用 `surface-2` |
| 图片卡 | `c-media` | 图片在上（16:9，8px 圆角）、下面是标签和日期、标题（三级标题，最多两行）；资讯列表上再加两行摘要（5.2）；整张可点；悬停时标题变 `primary-text`、图片放大 |
| 大图卡 | `c-feature` | 图片铺满，字压在图片下半部的 `scrim` 上：标签、标题（二级标题）、一行事实；首页的正在报名的赛事、赛事横幅用 |
| 列表行 | `c-row` | 左边日期块（`c-date`），中间标题和一行元信息，右边状态或日期；行间 1px `line`。`--plain` 没有日期块（搜索结果、账号安全）；`c-row__text` 是标题下一段正文色的摘录，最多两行 |
| 日期块 | `c-date` | 56×56，8px 圆角，`surface-2` 底：上面月份（小字），下面日（二级标题字号） |
| 进度条 | `c-meter` | 4px 高、4px 圆角，`surface-2` 底、`primary` 条，`<progress>` 元素；旁边用小字写「8 / 10」 |
| 数字条 | `c-stats` | 一行几个数字：数字 2rem 加粗，下面是名称（正文字号 `fg-2`） |
| 筛选 | `c-tabs` | 一行文字链接，当前项 `fg` 加 2px `primary` 下划线；手机上横向滚动 |
| 表格 | `c-table` | 1px `line` 描边、8px 圆角，表头 `surface-2`，行间分隔线，数字右对齐；手机上每行变成一段 |
| 资料表 | `c-facts` | 左键右值，行间分隔线 |
| 段位 | `c-rank` | 大段名 + 小段数字；未定级显示「未定级」 |
| 位置 | `c-roles` | 坦克、输出、支援三个自绘图标加文字 |
| 头像 | `c-avatar` | 圆形；用户没有头像，显示昵称首字 |
| 表单字段 | `c-field` | 标签在上，输入框高 44px、1px `control` 边框、8px 圆角；聚焦时 2px `primary`；帮助文字和错误文字用小字 |
| 分步 | `c-step` | 表单里的一组字段：`surface` 底描边框，上面一行三级标题，不编号 |
| 提示 | `c-notice` | 8px 圆角的 `-soft` 底色条：图标 + 文字 |
| 操作提示 | `c-toast` | 右上角（手机上是顶部）的深色条，4 秒后消失 |
| 空状态 | `c-empty` | `surface` 底描边框：一句话和至多一个操作 |
| 面板 | `c-panel` | 一组必须圈起来的内容：`surface` 底、1px `line`、8px 圆角；整个框都是链接时叫 `c-card`，悬停时描边加深 |
| 列表框 | `c-rows` | 一组 `c-row` 放在一个 `surface` 底、1px `line` 的框里 |
| 战队 | `c-teams` | 首页战队：队标（圆形）、队名、人数、「招募中」，整格可点 |
| 分页 | `c-pager` | 上一页、`2 / 7`、下一页 |
...
**样张页**：`/_styleguide/` 把上面每个组件的每种状态排在一页上，只有能进后台的账号能看，其他人 404；不预渲染、不进站点地图。改组件先在样张页上看。

#### 13.2.7 页面骨架

| 骨架 | 结构 | 用在 |
|---|---|---|
| **首页** | 首屏 → 数字条 → 近期 → 资讯与公告 → 战队（5.2） | 首页 |
| **刊物列表** | 栏目头 → 筛选 → 图片卡网格（桌面三列、平板两列、手机一列）→ 分页；搜索结果是搜索栏下按类分组的 `c-rows --plain` | 资讯、搜索结果 |
| **刊物正文** | 44rem 正文栏居中：面包屑、分类、标题、作者和日期、头图、正文、关联赛事、评论；正文后面是同栏目最新的一行图片卡。关于、用户协议、隐私政策三页标题上方是一行 `c-tabs` 互相切换 | 文章、协议、关于 |
| **赛场列表** | 栏目头 → 进行中和即将开始的用图片卡网格 → 已结束的用表格 | 赛事、内战 |
| **赛场详情** | 大图横幅（`c-feature`：标题、状态、关键事实、报名按钮）→ 主栏说明和名单 + 侧栏资料表 | 赛事、内战、战队主页 |
| **名册** | 栏目头 → 分组 → 头像网格 | 成员展示 |
| **表单** | 栏目头 → 字段 → 底部操作 | 报名、创建战队、申请入队、登录注册 |
| **个人中心** | 左侧菜单（手机上是筛选条）+ 右侧内容 | `/me/` 下的页面 |

各页用哪些区块：首页见 5.2 节；赛事 8.2；内战 9.2；战队 7.2、7.6；成员 6.3；个人中心 13.5。

#### 13.2.8 实现规则

- 模板只用语义颜色和组件类，不写色值、不写字体名；不写行内样式和行内脚本（内容安全策略，已有测试扫描）。进度用 `<progress>`
- 新组件先写进 13.2.6 的表和样张页，再在页面里用
- 图标全部自绘（`components/icon.html`），24×24、线宽 1.75、圆头圆角；不引入图标库
- **站点标志**：深红圆角方块，里面白色的向上折线，旁边是「交大守望先锋」
- **校徽**：文件 `static/img/sjtu-emblem.svg`（来源见 `THIRD_PARTY_NOTICES.md`），只用在首页首屏没有图片时的底（13.2.5）
- 前台模板里不能出现 daisyUI 的类名，有测试扫描
- 构建仍用 django-tailwind-cli（只用编译器，不加载 daisyUI 插件）

### 13.3 全站布局

- **页头**（`c-masthead`）：`surface` 底，底边 1px `line`，高 64px，`position: sticky` 贴在顶部，滚动时不变形、不收起。左边站点标志；中间主导航：首页、资讯、赛事、内战、战队、成员；右边搜索按钮（放大镜图标，去 `/search/`）和账号区域
- **账号区域**：未登录时显示「登录」（文字按钮）和「注册」（主要按钮）；登录后显示首字头像和昵称，点开是个人中心、我的报名、退出（`<details>` 下拉，不靠脚本）
- **手机端**：页头只留标志、搜索按钮、「登录」（或头像）和菜单按钮；菜单是在页头下面展开的面板（`<details>`）
- **页头上方**：测试环境横幅、微信内置浏览器提示（16.9、16.10）各占一整行
- **页脚**（`c-footer`）：左边标志和一句说明（学生社团自办网站，不是上海交通大学官方网站，与游戏开发商、运营商无关）；右边四栏链接：社区（资讯、成员、战队）、参与（赛事、内战、投稿）、账号（个人中心、登录、注册）、关于（关于我们、用户协议、隐私政策）；最下面一行版权和「游戏图片版权归暴雪娱乐所有」
- **操作提示**：`c-toast`，由 Django 消息框架和 HTMX 的 `HX-Trigger` 共同驱动；在预渲染页面上通过页面状态片段显示（13.13.3 节）
...
| `/_styleguide/` | 设计体系样张（13.2.6 节），实时渲染 | 能进后台的账号，其他人 404 |
...
- 页头是一级标题，下面一行是首字头像和昵称，不写眉标
- 左侧（手机端为顶部横向滚动的标签）菜单，每项一个图标（v3.0 起代替 `01`–`07` 编号），当前项用 `primary-soft` 底色标出（手机上是 `c-tabs` 的当前项，13.2.6）：基本资料、游戏 ID 与段位、联系方式、我的战队、我的报名、我的内战、账号安全
...
- 大号数字用 `--font-figure`（13.2.3）：默认等于 `--font-numeric`（v3.0 起不再用系统自带的 DIN 类字体）。「数字与数据」区域选了字库里的字体时，生成的样式表同时写 `--font-figure`，让大号数字也换成那个字体
...
- 错误页面用 13.2 节的颜色和字体令牌（`error.css` 是手写的，浅色和深色两套值都和 `input.css` 手工保持一致，有测试比对），跟随系统深浅色；大号错误码下面直接是中文标题，不写英文标签；不包含个人相关区域
...
| 8 | ~~视觉风格和主题色~~ | **已决定（2026-09-26）**：此前所有方案作废，由 Claude 从零定一套（v4.0，13.2）：中性底色、深红强调、图和内容先行、不做装饰；可以用游戏官方素材（后台上传，不进仓库） | 已确定 |
| 9 | ~~是否支持深浅色切换~~ | **已决定（2026-09-26）**：深浅两套，跟随系统（`prefers-color-scheme`），不做切换按钮（13.2.1）。推翻之前的「只做浅色」 | 已确定 |
...
| design v3.0.1 草案 | 2026-09-26 | M9 收尾（085）：13.5 个人中心页头改成头像加昵称、菜单编号换成图标；13.2.7 补 `c-sidenav`、`c-auth`、`c-why`，栏目头不写英文眉标 |
| design v4.0 草案 | 2026-09-26 | 按用户决定，前台视觉第三次重做：v3.0 和当天十余版样稿全部作废，从零定一套。13.2 节重写（约束、原则、深浅两套颜色、字号最小 14px、版式与动效、图片与官方素材、组件表、页面骨架、实现规则）；13.3 节页头改为不变形的实色条、页脚加游戏图片版权声明；5.2 节首页改为首屏图、数字条、近期、资讯与公告、战队，去掉近期安排卡、快捷入口磁贴、活动统计卡和数字递增；文章页正文栏居中、同栏目最新放到正文后面；12.4.1 新增 `hero_image`；13.13.4 首屏图片修改也刷新首页；19.2 第 8、9 条改写（深浅色跟随系统） |
