...
| 图片卡 | `c-media` | 图片在上（16:9，8px 圆角）、下面是标签和日期、标题（三级标题，最多两行）；整张可点；悬停时标题变 `primary-text`、图片放大 |
| 大图卡 | `c-feature` | 图片铺满，字压在图片下半部的 `scrim` 上：标签、标题（二级标题）、一行事实；首页的正在报名的赛事、赛事横幅用 |
| 列表行 | `c-row` | 左边日期块（`c-date`）或小图，中间标题和一行元信息，右边状态或日期；行间 1px `line` |
...
| **刊物列表** | 栏目头 → 筛选 → 图片卡网格（桌面三列、平板两列、手机一列）→ 分页 | 资讯、搜索结果 |
| **刊物正文** | 44rem 正文栏居中：面包屑、分类、标题、作者和日期、头图、正文、关联赛事、评论；正文后面是同栏目最新的一行图片卡 | 文章、协议、关于 |
