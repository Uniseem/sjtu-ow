...
| 02 赛事与内战（赛场，深色） | 上面是**日程带**：从今天起 14 天（**默认**），每天一格，有公开或已结束的内战、有赛事开赛的日子标出数量；下面左边是正在报名和即将开放报名的赛事（票根，最多 3 张），右边是日程带里的内战列表 |
...
| 评论 | `c-comments` | 方形首字头像；回复串缩进并用发丝线连起来 |
...
| 骨架 | 结构 | 用在 |
|---|---|---|
| **刊物列表** | 栏目头 → 标签条 → 主栏列表行 + 侧栏 → 分页 | 资讯、搜索结果 |
