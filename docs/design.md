...
- 下载一个 JSON 文件，包含：账号信息（邮箱、昵称、是否来自交大、注册时间、同意用户协议和跨境存储的时间）、游戏 ID 和段位、联系方式、所在战队和身份、入队申请、赛事报名（自己在名单里的，含名单快照）、内战报名、所在的成员分组和职务、署名的文章（标题和地址）
...
| 快速入口 | 成员展示、战队、赛事报名、内战报名、社区资讯、我要投稿、个人中心、关于我们 |
...
- 版式和其他内页一样：左边米色侧栏列出各分组（点击跳到对应区块）和「全部成员」
- 按分组的排序，每个分组一个区块：分组名、简介、成员卡片（昵称、职务、战队）；组内按排序，再按昵称
- 最后是「全部成员」：所有已加入的用户，按加入时间先后排列，显示昵称和所在分组
- 还没有任何分组时，只显示「全部成员」
...
- **页头**（交大红）：左边站点标志和名称；右边上一行是常用入口（赛事报名、内战报名、成员展示、我要投稿），下一行是细白框里的主导航：首页、资讯、投稿、赛事、战队、内战、成员。当前栏目高亮
...
| 用户修改昵称 | 该用户所在的战队主页、报名的内战活动详情、署名的文章、成员展示 |
...
| 前台业务操作限流 | 申请入队每人每天 20 次；创建战队每人每天 3 次；页面状态片段每个 IP 每分钟 120 次（取 `X-Forwarded-For` 最后一段，即 Caddy 记录的来访 IP） | 配置 |
