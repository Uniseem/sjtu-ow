...
- 下载一个 JSON 文件，包含：账号信息（邮箱、昵称、是否来自交大、注册时间、同意用户协议和跨境存储的时间）、游戏 ID 和段位、联系方式、所在战队和身份、入队申请、赛事报名（自己在名单里的，含名单快照）、内战报名、车帖、署名的文章（标题和地址）
...
版式照上海交大官网首页做（13.2 节），从上到下：

| 区块 | 内容来源 |
|---|---|
| 焦点图 | 顶部轮播，每 5 秒换一张（**默认**）。后台在首页里维护「焦点图」：图片、标题、链接，最多 6 张；一张都没有时，取最近 5 篇带封面的文章；再没有就不显示 |
| 图片新闻 | 最近 4 篇带封面的文章（**默认**） |
| 社区要闻 | 置顶文章排在最前，后面接最新文章，共 7 篇（**默认**）。置顶文章在后台指定，最多 3 篇 |
| 赛事通知 | 正在报名的赛事和未来 7 天内的内战（**默认**），各一行 |
| 组队大厅入口 | 放在赛事通知下面。已登录用户看到当前招人中的车帖数量（动态加载，13.13.3 节）；未登录访客看到「登录后查看」 |
| 赛事 | 卡片一行最多 3 张，先放正在报名的赛事，再放近期内战；旁边是「内战日历」，标出本月有公开内战的日子 |
| 战队 | 最新成立的 5 支战队（**默认**），拱形卡片，显示队标（没有就显示队名前两个字） |
| 快速入口 | 组队大厅、我要发车、赛事报名、内战报名、战队、社区资讯、我要投稿、个人中心 |

**文章页**只显示标题、发布日期、正文，文末右对齐写作者和栏目；摘要只在列表和分享信息里用。正文里的「引用」块用楷体、左右各缩进两格，出处写在下面「——出处」。
...
**用户决定（2026-09-19）：照上海交通大学官网（www.sjtu.edu.cn）的风格做。** 之前试过 HeroUI 那种卡片风格，用户认为「不正式、AI 味重」，放弃。

- **颜色**：主色是官网页头的交大红 `#B2141A`，顶栏更深（`#6B0004`）；大块浅色区域用米色 `#F8F2EA`；装饰用官网首页的玫瑰色 `#CF8F90`、沙色和古铜色。只在 CSS 入口文件的 daisyUI 自定义主题 `sjtu` 和几个 CSS 变量里定义
- **字体**：默认宋体（衬线），和官网一致；后台排版设置仍然可以换
- **形状**：直角或很小的圆角，不用阴影和渐变装饰
- **只做浅色**，不跟随系统深色模式，也不做切换（19.2 第 9 条）
- **不用交大的校徽、书法校名和照片**。站点标志是自己设计的印章样式（「交大守望」四个字，按传统从右列读起）；页脚写明「学生社团自办网站，不是上海交通大学官方网站」
- 页面结构照官网：首页见 5.2 节；内页左边是米色的栏目侧栏，右边是内容；文章页照交大新闻网的文章页

以下规则从第一行模板就遵守，保证以后换风格不用改模板：
...
- **顶栏**（最上面一条深红色）：左边「学生社团自办网站」，右边账号区域
- **页头**（交大红）：左边站点标志和名称；右边上一行是常用入口（赛事报名、内战报名、我要发车、我要投稿），下一行是细白框里的主导航：首页、资讯、投稿、赛事、组队大厅、战队、内战。当前栏目高亮
- **账号区域**：未登录时显示「登录」「注册」；登录后显示昵称、个人中心、退出
- **手机端**：导航折叠进侧边抽屉菜单（daisyUI 的 drawer 组件）
- **页脚**：米色竖条纹底，中间是站点标志、名称和「不是上海交通大学官方网站」的说明，两边是链接（社区各栏目；关于我们、用户协议、隐私政策、个人中心）
...
| 战队创建、修改、成员变动、招募状态变化、解散 | 战队主页、战队列表、首页（战队区块） |
...
| 8 | ~~视觉风格和主题色~~ | **已决定（2026-09-19）**：照上海交大官网，主色交大红，见 13.2 节 | 已确定 |
| 9 | ~~是否支持深浅色切换~~ | **已决定（2026-09-19）**：只做浅色，和官网一致 | 已确定 |
...
| 前台业务操作限流 | 发车每人每小时 10 次；申请入队每人每天 20 次；创建战队每人每天 3 次；页面状态片段每个 IP 每分钟 120 次（取 `X-Forwarded-For` 最后一段，即 Caddy 记录的来访 IP） | 配置 |
