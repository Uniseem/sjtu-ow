...
前台按设计 13.2 节的「设计体系」做（v3.0，081 起）：参照 Android 开发者站和 AOSP 站的 Material Design 3，交大红是种子色；浅色、色调分层、圆角和胶囊按钮，系统字体；只有顶栏和首页近期安排卡是磨砂玻璃。顶栏往下滑变成胶囊并收起、往上滑弹出（`static/js/motion.js`）。首页首屏用交大校徽做水印（`static/img/sjtu-emblem.svg`，来源和权利见 `THIRD_PARTY_NOTICES.md`）；页脚照旧写明不是官方网站。

- 颜色、字体变量、全部组件（`c-*`）和布局类（`l-*`）只写在 `assets/css/input.css` 里；模板只用语义颜色（`text-fg-2`、`border-rule`、`bg-surface-high`）和组件类，**不写色值**。半透明色也用令牌混（`color-mix`），不写 `rgb()`。Tailwind 自带的色板已经关掉，写 `bg-orange-500` 不会生效
- **对比度**：文字至少 4.5:1、输入框这类控件的边框至少 3:1（WCAG AA）。设计 13.2.3 列了必须达标的前景 / 底色组合，测试从 `input.css` 读令牌逐对计算；`@theme` 以外写了色值、或者样张页的色块和令牌对不上，测试也会红。改颜色先改设计的令牌表
...
首页版式见设计 5.2 节（v3.0，082 起）。后台要维护的：

- **「设置 → 全站设置 → 站点信息」**：**社区成立日期**（首页「社区已成立 N 年 M 天」按它算，每天凌晨的全量重新生成会让天数跟着走）、**QQ 群链接**（`https://` 开头，首页快捷入口「加入 QQ 群」链接到这里）。两项都可以空着，空着首页就不显示对应的那块
- **「页面 → 首页 → 编辑 → 置顶文章」**：最多 3 篇，排在首页「资讯」最前面，带「置顶」标签

注册成员、战队数、近期安排（报名中的赛事 + 未来 7 天的内战）、通知公告（「公告」「赛事通知」两个分类最新 5 篇）、活动统计（已结束的内战场数、赛事届数）、战队都是自动取的。v2.0 的焦点图在 082 删了。**文章有封面才会在资讯头条里显示图片**，没有封面用一块色彩渐变代替。
