...
- **焦点图**：首页左上的轮播，最多 6 张，裁成 16:9。每张填图片（横图，1600×900 以上）、标题、链接（选一个页面，或填以 `/` 开头的站内地址、`https://` 开头的站外地址）。一张都不填时，自动用最近 5 篇带封面的文章
- **置顶文章**：最多 3 篇，排在「01 资讯」最前面，带「置顶」标签

「近期」、日程带、赛事和内战的票根、战队图块都是自动取的。**文章有封面才会进焦点图（没有手工焦点图时）和资讯列表的缩略图**，发文章时尽量配一张横图（16:9 最合适）。
