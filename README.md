...
- **个人报名（散人池）**：赛事打开「开放个人报名」后，不是队长的用户在赛事页点「个人报名」，到 `/tournaments/<id>/signup/` 选游戏 ID、勾能打的位置（校验和队员一样：权限、资料完整、仅限交大、不在别的名单里；不要求段位）。赛事页公开人数、昵称和位置，不显示段位和游戏 ID；截止前可以改或取消；个人中心「我的报名」里也能取消。编队由赛事管理员在后台完成（070 起）。
