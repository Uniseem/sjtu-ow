...
前台按设计 13.2 节的「设计体系」做（v4.0，086 起）：中性的白和灰做底，深红只用在主要按钮、当前导航和「报名中」；图和内容先行，不做装饰（没有渐变色块、玻璃、水印、进场动画）；字不小于 14px。**深浅两套跟随系统**，浏览器或手机切到深色模式时整站变深色，没有切换按钮。页脚写明不是官方网站、游戏图片版权归暴雪娱乐。

- 颜色、字体变量、全部组件（`c-*`）和布局类（`l-*`）只写在 `assets/css/input.css` 里：浅色值在 `@theme`，深色值在紧跟着的 `@media (prefers-color-scheme: dark)`。模板只用语义颜色（`text-fg-2`、`bg-surface-2`）和组件类，**不写色值**；半透明色用令牌混（`color-mix`）。Tailwind 自带的色板已经关掉
- **对比度**：两种模式下文字都至少 4.5:1、输入框这类控件的边框至少 3:1（WCAG AA），测试从 `input.css` 读令牌逐对计算。改颜色先改设计 13.2.2 的令牌表
- 日期、时间、星期、名额的格式统一用 `core/templatetags/ow.py` 的过滤器（`{% load ow %}`）
- **样张页 `/_styleguide/`**：每个组件的每种状态排在一页上，能进后台的账号登录后可以看，其他人 404。改组件先在这里看
- 只改模板、用了新的工具类时要 `tailwind build --force`（见 `AGENTS.md` 的坑）
- 前台不用 daisyUI，模板里写了 daisyUI 的类名会被测试拦下

首页版式见设计 5.2 节（v4.0，086 起）。后台要维护的：

- **「设置 → 全站设置 → 站点信息」**：**首屏图片**（首页最上面的大图，建议 2400×1000 以上；空着时是深色底加交大校徽）、**社区成立日期**（数字条里的「社区已成立 N 年 M 天」，每天凌晨的全量重新生成会让天数跟着走）、**QQ 群链接**（`https://` 开头，首屏的「加入 QQ 群」按钮链接到这里）。都可以空着，空着就不显示对应的那块
- **「页面 → 首页 → 编辑 → 置顶文章」**：最多 3 篇，排在首页「资讯」最前面，带「置顶」标签
- **图片**：可以用守望先锋的官方图片（用户 2026-09-26 决定），但只在后台上传，**不要把暴雪的图片提交进仓库**（仓库是公开的）。赛事封面、文章封面、队标都会出现在首页；没有图时用一块灰底写上赛事名或分类名

数字条（注册成员、战队、累计内战、社区已成立）、近期（报名截止最近的一场赛事 + 未来 7 天最多 4 场内战）、资讯（置顶加最新共 4 篇）、公告（「公告」「赛事通知」两个分类最新 5 篇）、战队（最新 6 支）都是自动取的。
...
- 文章页底部，登录后可发评论、回复；回复的回复仍挂在同一条顶层评论下，正文前带「@昵称」。正文最多 500 字，纯文本。每人每分钟 3 条、每天 100 条；功能权限 `article_comment` 可对某组或某人禁言。
- **先发后审**：每条评论送 AI 审核（`comment` 类型），疑似问题进后台「内容审核」；内容编辑在文章页或后台「社区 → 评论」里隐藏、恢复。隐藏的评论读者看不到；有可见回复的隐藏顶层评论显示成占位。
- 每篇文章的编辑页有「开放评论」开关（投稿者看不到）；关闭后已有评论仍显示。
- 文章页是预渲染的：静态页里只有只读列表和「登录后评论」；登录用户由页面加载后的一次片段请求换成带表单的版本。评论的每次变化都重新生成文章页。
- **点赞与排序**：登录后可给可见的评论点赞、取消（每人每分钟 60 次）；「最新」按时间倒序，「最热」按赞数、可见回复数、时间；置顶的永远在最前，每篇文章只有一条置顶（内容编辑在文章页或后台设，置顶新的自动取消旧的；隐藏或删除会取消置顶）。
- **自己的评论**可以编辑（标「已编辑」，重新送审）和删除（正文清空、记录保留；有回复的顶层评论显示成「评论已删除」占位；删掉的不能再赞、再回复）。
